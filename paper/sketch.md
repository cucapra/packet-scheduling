...
A change to the live control is _atomic_ when its effect falls between two observable operations: in any sequence of `push`/`pop` operations `op_1, op_2, ...` served by the scheduler, if the change fires between `op_N` and `op_{N+1}`, then `op_1, ..., op_N` are served entirely by the pre-change control and `op_{N+1}, ...` entirely by the post-change control. No operation straddles the change, and no operation sees an intermediate state. §1's running example described this informally. §3.2 fixes a grammar of structural edits (_diffs_), every one of which is atomic by construction; §3.3 restates atomicity once the operational rewrite `[[δ]]` is in hand and proves that it holds for every kind of `diff`; §3.4 argues that atomicity survives the lowering to hardware.
...
##### Discipline compilation: `init_D`

For each discipline `D`, `init_D` is the mechanical recipe that takes `D` to (a) a per-node ranking program contributing to `z`, (b) an initial `node_state` for the node, and (c) a `slot_state` for each arm attached under the node. We treat slot-seeding as its own typed projection of `init_D`:

```
init_D : node_state × pol × weight? -> slot_state
```
...
[AM note: `⌊C⌋` as defined here is literal only on freshly-compiled controls. Once `C` has been edited (a `Quiesce` has shrunk `z`'s domain, a `Designate` has rewired `z`, etc.) the live `C` is no longer the output of `compile` on any single `pol`, so "the pol from which `C` was compiled" stops being well-defined. §3.3's realization equation `⌊[[δ]](C)⌋ = den(δ)(⌊C⌋)` nonetheless treats `⌊·⌋` as defined on post-edit controls. Two possible repairs: define `⌊·⌋` as a tag the realization equation propagates from the freshly-compiled control onward, or define it as a derived function that reads topology, disciplines, weights, and labels off `C` directly. Discuss IRL.]

### 3.2 A Grammar for Tree Diffs

The user writes down `pol` objects, like `prev` and `next`. Our differ compares two `pol`s. Say that we compile the user's request `prev` into some control `C`, s.t. `C` realizes `prev` (written `prev = ⌊C⌋`). Then the user requests a change to a new `pol`, `next`. SOTA would compile `next` into some control `C'` and clobber the running `C` with `C'`. We wish to achieve the same result while being less disruptive to unaffected parts of the running control.

We fix a small grammar of atomic edits. Each production is a single primitive that acts on a live control `C = (s, q, z)` and produces a new control. The grammar is the alphabet from which the planner (§4-§5) assembles a sequence whose operational composition takes `C` to a control realizing `next` _without the clobber_. Most productions are pol-visible: their effect shows up in `next`, and a comparison of `prev` against `next` is enough to read them off. Others are transaction-only: their effect lives entirely in `z`, leaving the pol-level skeleton untouched (the planner emits them anyway, for operational safety; see §4).
...
- `Designate(path, pol)` converts `prev@path` into `Strict(prev@path, pol)` in place. The existing subtree becomes the high-priority sibling (set to drain), and `pol` becomes the low-priority _designated survivor_. Operationally, `z` is rewired so that traffic that used to classify into `prev@path` is now _offered_ to the survivor instead: the survivor's own classifier decides what to do with each packet, accepting any it has a leaf for and dropping the rest via the partial-`z` extension of §3.1.2. We make no assumption about how much overlap there is between `prev@path`'s flows and the survivor's. The drain side serves only the packets buffered before the edit and is collapsed by a closing `Remove` once empty (§5). Where the survivor exposes a leaf whose label coincides with one under `prev@path`, the differ names that survivor leaf with a fresh primed label (`A'` is read as "packets that would have gone to `A` before the edit") so the post-edit `pol` has, by construction, disjoint leaf labels and remains valid. The production does not need to carry a weight even when the parent at `path`'s prefix runs WFQ, since the new `Strict` node inherits `prev@path`'s slot and weight, and the survivor competes against `prev@path` strictly _inside_ that wrapper.
...
- every `path` is consistent with `prev`'s shape: resolving to an existing slot for `Quiesce` / `Designate` / `Remove` / `ChangeWeight`, and to a fresh slot of an existing internal node for `Add`;
...
- for an `Add`, the classifier domain of `pol`'s compiled `z` is disjoint from the domain of `C`'s live `z`.

The last condition is a semantic side condition on `C`'s live `z`, not enforceable by labels alone: it is what licenses the prose-level claim that an `Add` introduces "fresh traffic that intersects nothing `prev` was already serving" (§3.2), and it is what lets the operational transition of §3.3.1 extend `z`'s domain without colliding with `C`'s routes. Throughout §3.3, the two semantic readings of `δ` developed below are stated on `(δ, C)` pairs that satisfy this condition; §4's transition planner emits only such pairs by construction.
...
- The _pol-level denotation_, `den(δ) : pol -> pol`, is the projection of `[[δ]]` through the `⌊·⌋` bridge of §3.1. It captures the pol-visible part of the edit. For most primitives (`Add`, `Remove`, `ChangeWeight`, `Graft`, and `Designate`'s wrap-and-rename), `den(δ)` is non-trivial and matches what a reader would extract from comparing `prev` against `next`. `Quiesce`, whose whole effect is in `z`, has `den(Quiesce) = id_pol`. The z-rewiring inside `Designate` lives only in `[[Designate]]`, not in `den(Designate)`. A `pol` carries only topology, disciplines, weights, and leaf labels (no live contents and no transaction), so `den(δ)` is a purely static map, and effects that live entirely in `z` are invisible to it. This is the reading §4's planner reasons against when comparing user-supplied `prev` and `next`: a sequence of atomic edits realizes the reconfiguration when its composed `den` at the pol level lands at `next`, with pol-invisible edits like `Quiesce` acting as identities along the way.

The two readings are linked through `⌊·⌋`: `⌊[[δ]](C)⌋` and `den(δ)(⌊C⌋)` are two ways of asking the same question (_what `pol` does the post-edit control realize?_), and they had better agree.

In the language of `[[δ]]`, atomicity (defined at the top of §3) reads: `[[δ]]` replaces the live control `C` with `[[δ]](C)` between two consecutive `push`/`pop` operations. Modeling `[[δ]]` as a single function `control -> control` builds the indivisibility into the abstraction; the obligations below ensure that the result of this single step is correct.
...
We discharge the three obligations of §3.3 in turn, writing `C_next = [[Add(path, pol, weight?)]](C_prev)`.
...
- _State preservation._ At every position outside the new subtree and other than `path_c`, `s'(pos) = s(pos)` by the first case of the state update; this discharges the "structurally shared and outside the edit site" clause of §3.3. At `path_c` the `node_state` is unchanged and the `slot_state` vector splices in `init_D(s(path_c).node_state, pol, weight?)` at index `k`; this is exactly the `init_D` invocation §3.1.2 prescribes for splicing a fresh arm under a running `D`-parent, matching the "at the edit site" clause. Inside the new subtree, `s'` is the freshly-compiled `s_pol`, which is just `init_D` applied node-by-node through `pol` per §3.1.2's compilation rule, matching the "inside a freshly-spawned subtree" clause.
