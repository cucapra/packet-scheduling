...
We need one small extension to _FA_'s formalism. _FA_ describes `z` as a total function `St × Pkt -> Path(t) × St`; we describes it instead as a partial function `z : St × Pkt ⇀ Path(t) × St`. An entry on which `z` is undefined is one where the packet is being dropped: no path is emitted, no state update occurs, `q` is left alone. _FA_'s total typing is the special case where the domain is all of `St × Pkt`, and every _FA_ result we lean on carries through verbatim: a dropped packet never enters `q`, so `q`'s well-formedness obligations are unaffected.
