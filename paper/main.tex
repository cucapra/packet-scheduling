...
The policy on a switch must not be fixed.
Traffic patterns shift, tenants arrive and leave, and telemetry downstream tells the network operator that the policy is no longer apt.
But the scheduler is compiled expressly for the policy it runs: its tree shape, per-node disciplines, and table contents are selected to realize \emph{a fresh policy}, not \emph{a change to the running policy}~\cite{Sivaraman16,Gao22,vPIFO}.
When the operator requests a new policy, the freshly compiled scheduler is agnostic to the running policy and the backlog of buffered packets~\cite{vPIFO,Alcoz23}.
The whole tree is rebuilt, including the nodes that could have been carried over unchanged.
What becomes of the backlog is up to the implementation: it can be dropped, recirculated, or left to be scheduled through a chaotic period in which the hardware follows no clearly defined policy.
Each of those is itself tacitly a \emph{scheduling decision}, and the operator is unable to specify which one they want.
