...
The policy on a switch cannot be fixed.
Traffic patterns shift, tenants arrive and leave, and telemetry from downstream tells the network operator that the running policy in no longer apt.
But the scheduler on a switch is compiled expressly for the policy it runs: its tree shape, per-node disciplines, and table contents are all selected to realize \emph{a fresh policy}, not \emph{a change to the running policy}~\cite{Sivaraman16,Gao22,vPIFO}.
When the operator requests a new policy, the freshly compiled scheduler knows nothing about the running policy or the backlog of buffered packets~\cite{vPIFO,Alcoz23}.
The whole tree is rebuilt, including the nodes that could have been carried over unchanged.
What becomes of the backlog is left to the implementation: it can be dropped, recirculated, or left to be scheduled through a chaotic period in which the hardware follows no clearly defined policy.
