...
An \texttt{assoc} writes the first, a \texttt{map} the second, an \texttt{adopt} the third, and a \texttt{designate} the fourth.
The lifecycle opcodes reach a PE too, since \texttt{spawn} and \texttt{gc} claim and release its PIFO storage.
Arities are how the compiler sizes a tree position and no PE stores one, so \texttt{change\_arity} reaches no PE at all.
