...
On programmable switches, network operators configure schedulers to isolate tenants from one another and to hold latency-sensitive traffic to its budget.
The policies they write are usually hierarchical~\cite{Stephens19,Kumar19,Grant20,vPIFO}, each level running a scheduling discipline of its own.
Whatever the policy, the scheduler must keep up with the wire.
A \emph{PIFO}, a priority queue that a switch can maintain at line rate, makes this affordable in hardware.
...
Traffic patterns shift, tenants arrive and leave, and telemetry from downstream tells the network operator that the running policy is no longer apt.
But the scheduler is compiled expressly for the policy it runs: its tree shape, per-node disciplines, and table contents are all selected to realize \emph{a fresh policy}, not \emph{a change to the running policy}~\cite{Sivaraman16,Gao22,vPIFO}.
