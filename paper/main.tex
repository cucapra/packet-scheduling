...
Specifically, \Cref{sec:controls} presents a formal description of a PIFO tree control, $C$, representing the state of a packet scheduler.
...
\Cref{fig:hardware} lays a PE out from left to right, with a match--action table at either end, and stacks the levels top to bottom.
...
A PE stores four things: its \texttt{Assoc} set, the flows it admits; a \texttt{Map}, read on enqueue, naming the PIFO each admitted flow enters and the PE that holds it; a next-hop table, read on dequeue, naming the PIFO that each of its indices reaches; and the designated-survivor link of \cref{sec:hw-survivor}.
...
\Cref{fig:hardware} draws that logic as the survivor-link cell sitting beside a PE's PIFOs rather than in the pipeline with them.
\xxx[am to zg]{Is the previous sentence okay? If I understand correctly, it \emph{is} pipelined, right?}
...
We ask what a transition costs the traffic that the operator's request does not concern, and what the additions cost in hardware.

\begin{figure}[t]
\centering
\includegraphics{fig-eval-motivating.pdf}
\caption{The transition of \cref{sec:tour}, as a share of link capacity. The middle panel is \textsf{zoom}, the flow the request never touches; the lower one is all three flows under Rio. The band is the interval in which Rio runs $p_\text{temp}$.}
\label{fig:eval-motivating}
\end{figure}

\paragraph{The baseline, and why it is generous}

Reconfiguring by stopping the scheduler admits many implementations, and we compare against a careful one.
Our stop-the-world baseline freezes admission and dequeue, captures the metadata of every buffered packet, installs the target configuration, and replays exactly one scheduler token per captured packet.
It therefore drops nothing and reorders nothing within a flow, exactly as \cref{sec:nondisruption} demands of Rio, and traffic sources keep generating while it is stopped.
Every mechanism below delivers every packet in per-flow order, so these experiments separate the mechanisms on the duration of a transition alone, and not on the dropping and recirculation that \cref{sec:intro} raises.
We also grant the baseline an optimistic stop: its floor of 1024 cycles is a parameter we set rather than a cost we measured, and at the one nanosecond per cycle we label throughout it is a little over a microsecond.
Reconfiguring the program that a running switch executes has been reported at around half a second~\cite{Xing22}, so the baseline is handed a stop far shorter than any published figure, and it is still the slower mechanism below.

\paragraph{The transition of \cref{sec:tour}}

The policies are the ones \cref{sec:tour} walks through, on three processing elements, with every leaf terminating at a hardware \FIFO{}.
\textsf{zoom} offers 0.40 of the link throughout, \textsf{gmail} 0.80 before the request and 0.20 after, and \textsf{spotify} arrives with the request offering 0.20.
Four mechanisms replay one 2136-packet trace and all four complete it with no drops and no within-flow reordering.
Beside the baseline, \emph{whole-tree replace} builds \(p_2\) fresh alongside the running tree and drains the old one behind it, and Rio's \emph{confined replace} installs the \(\Strictstar\) transitional policy and collapses it once \textsf{gmail}\textsubscript{old} is empty.
A single \(\Add\) serves as a do-nothing reference.
...
\midrule
single \(\Add\) (reference) & 7 & 14 & 20 \\
stop the world & 25 & 42 & 1055 \\
whole-tree replace & 26 & 34 & 613 \\
\textbf{confined replace (Rio)} & \textbf{16} & \textbf{21} & \textbf{21} \\
\bottomrule
\end{tabular}
\caption{The transition of \cref{sec:tour}, four ways.}
\label{tab:eval-motivating}
\end{table}

\Cref{fig:eval-motivating} separates two ways a transition can go wrong.
Stopping the scheduler empties the link, and the upper panel is flat at zero for as long as the stop lasts.
Whole-tree replace never idles the link; it misallocates it, which only the middle panel shows.
\textsf{zoom} sits at the same priority and offers the same rate in every policy here, so a mechanism that touches only what changed should leave its line flat, and only Rio's is flat.
Its peak delay is 21 cycles against the reference's 20, where whole-tree replace costs it 613 and the baseline 1055 (\cref{tab:eval-motivating}), and neither baseline recovers when its interval ends: one hands \textsf{zoom} the whole link to catch up, and the other is still running above its offered rate 3400 cycles after a stop that lasted 1024.
Rio's own transitional interval is no shorter.
The band is the 1018 cycles in which the switch runs \(p_\text{temp}\), a policy no operator requested, and it costs the traffic nothing: the link stays full and \textsf{zoom} is undisturbed from one end of it to the other.
The lower panel adds the two flows the request does change, against \textsf{zoom} for reference.
\textsf{zoom} holds the high-priority arm but asks for only 0.40 of the link and gets it, so \textsf{gmail} takes the remainder and runs above it throughout: strict priority fixes which packet leaves next, and not how the capacity divides.
\textsf{gmail} is lifted while \textsf{gmail}\textsubscript{old} drains ahead of new arrivals, and \textsf{spotify} waits on that drain, reaching first service 1045 cycles after the request against the baseline's 1228.
Serving the new flow sooner would mean relocating the buffered packets, which is the price \cref{sec:tour} declines to pay.
\xxx[am]{The third panel is Rio alone and measures nothing against a baseline; it illustrates \(p_\text{temp}\) rather than evaluating it.
Happy to move it to the appendix and drop the three sentences that read it, if \S8 needs the space.}
Confinement is also the cheapest of the three to install, since it rewrites one subtree and leaves the rest of the control addressed as it was.

The \(\Strictstar\) node keeps the stop off that critical path.
Reaching the same transitional policy without one means materializing the wrapper and writing a token for every buffered packet, which we measure at 0.990 stopped cycles per packet on a reserved element and 2.975 when the subtree is copied, against zero for a survivor link at every backlog from 0 to 507 packets.
A survivor link is a pointer, so its cost does not depend on how busy the switch was when the request arrived.

\paragraph{A larger reconfiguration}

A fourteen-flow tree on seven processing elements takes one request that reweights two tenants, admits four, and retires a fifth.
Rio installs it in 51 instructions with 20 more behind a drain guard, against 152 for the reset baseline and 201 for whole-tree prefill.
Rio's instruction count tracks the part of the tree the request names, where both baselines rebuild a tree whose size has nothing to do with what was asked for.
Rio stops the scheduler for nothing; the two baselines buffer 624 and 495 packets during theirs.
...
\bottomrule
\end{tabular}
\caption{Synthesis overhead on a Xilinx XCKU5P, five processing elements, 1024 PIFO entries each.}
...
Synthesizing for an Agilex 7 part shows the same asymmetry at a higher logic cost, 18\% more ALMs against 62\% more block memory.

\paragraph{What these experiments do not show}

The transitions above were compiled by a harness that implements each mechanism directly, so no run here exercises the planner of \cref{sec:planner}.
We verified that the sequence measured for the larger reconfiguration is the one \cref{sec:delta-grammar}'s grammar prescribes for that request, so the claim these runs support is about the edits and not about the search that finds them.
The baselines also need hardware Rio does not, including a global pop stop and a datapath that copies a processing element's entries; all of it lives in a separate elaboration target that a build check keeps out of the image Rio runs on.
The resource numbers are sums of synthesized components without placement or routing, so they are not a timing closure, and the denser configurations of \cref{tab:eval-resources} exceed the part's memory.
...
The \texttt{adopt} creates that slot and the \texttt{set\_arm\_meta} initializes it, which together are the lowering's counterpart of \(\mathit{init\_slot\_state}_{D}\) in \cref{sec:add}.
...
An \texttt{emancipate} at \texttt{parent}'s index \texttt{k} detaches the arm, an \texttt{unmap} walk over \(\mathtt{internals(f)}\) for each \(f \in \mathtt{flows(v)}\) forgets its routing, and a \texttt{gc} at every PIFO of the departing subtree releases the PE slots.
...
foreach f in flows(surv):
    ...
    walk(map,   internals(f), f)
\end{Verbatim}
This is \Add{}'s lowering, with \texttt{designate} in place of \texttt{adopt} and one walk in front.
