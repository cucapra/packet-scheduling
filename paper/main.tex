...
\title[Live Reconfiguration of Hierarchical Packet Schedulers]{Live Reconfiguration \\ of Hierarchical Packet Schedulers}
...
In cycle-level simulation, Rio never pauses the scheduler to install a transition, and unaffected flows suffer only a single cycle of delay (\cref{sec:eval}).
In contrast, naively pausing the scheduler to install the same change costs that unaffected flow more than a thousand cycles.
When synthesized for an FPGA, the two hardware extensions add minimal logic overhead and acceptable memory overhead.

\section{Rio by Example}\label{sec:tour}

\Cref{fig:tour} illustrates Rio's approach to implementing an example update to a packet-scheduling policy.
...
\Cref{sec:eval} returns to this example in cycle-accurate simulation, where the transition above keeps the link full and leaves \textsf{zoom} undisturbed during \(p_\text{temp}\).
...
For Rio's purposes, a PIFO \emph{substrate} is the hardware that buffers the packets, hosts the tree of PIFOs, and decides which packet to release next.
Configuring one means supplying the shape of the tree, the ranking logic that each node runs, and the tables that tell a node which of its children a given flow descends into.
A substrate is programmable, so an operator can install a new configuration at any point, but the installation is defined for a scheduler that is not in the middle of serving traffic.

To the best of our knowledge, no prior work on PIFO trees says how to move a running scheduler from one configuration to another without disturbing buffered traffic.
%
...
We introduce the high-level programming interface for Rio.
...
\item They are \emph{non-disruptive}. Installing an edit never drops, reorders, or re-ranks a packet that is already buffered (\cref{sec:nondisruption}). A stop-the-world reset is explainable and complete but is disruptive. Edits that delete a subtree, such as $\Remove$, have a precondition that requires the doomed subtree to be empty; deleting an occupied subtree has no canonical realization, and \cref{sec:remove} shows why.
\end{enumerate}
%
An edit describes where in the tree the change lands and what the change is.
A \pth is a sequence of child indices \(i\) read from the root.
\(p@\mathit{path}\) denotes the subtree of $p$ at \pth.
The node that an edit's \pth names is its \emph{edit site}.

Informally, the six forms do the following.
\Add{} splices a freshly compiled arm under the internal node at the edit site.
\ChangeMeta{} rewrites the metadatum on one arm.
...
We formalize PIFO tree hardware (\cref{sec:controls}),
...
The triple has three components.
\stt is a pair \((\mathit{node\_state}, \mathit{slot\_states})\): the scheduling discipline's per-node bookkeeping and a list of per-arm entries, one per child.
Both evolve as the node serves traffic, and \slotstates is empty when the node runs a discipline that keeps no per-arm bookkeeping.
\pifo is a PIFO of ranked \emph{payloads}, a payload being what a node enqueues when presented with a packet: an index into the child list at an internal node, and the packet itself at a leaf.
\zz is the \emph{scheduling transaction} \(\mathit{z} : \mathit{state} \times \mathit{Pkt} \rightharpoonup (\mathit{payload} \times \mathit{rank}) \times \mathit{state}\).
...
\zz is partial, and when it is undefined for a packet, nothing is enqueued at this node and \stt is unchanged.

\zz has four parts, which the edits of \cref{sec:delta-grammar} rewrite separately.
Its \emph{admitted set} (\(\mathit{z}.\mathit{admit}\)) is the flows the node currently accepts, its \emph{service map} (\(\mathit{z}.\mathit{serves}\)) sends each flow the node serves to the payload that carries it onward, its \emph{per-arm metadata} (\(\mathit{z}.\mathit{meta}\)) records what the node's discipline needs to know about each arm, and its \emph{ranker} (\(\mathit{z}.\mathit{rank}\)) is the scheduling logic that discipline contributes.
\Cref{sec:edit-semantics-full} defines the four and shows how \zz composes them.

%Two of their consequences carry through the rest of the paper.
%A node admits a subset of the flows it serves, and keeping the two apart lets a silenced subtree go on draining (\cref{sec:quiesce}).
%A node's ranker names the discipline that the node runs, so the discipline is recoverable from a running control rather than lost at compile time (\cref{sec:equivalences}).
...
A control exposes two operations to the network.
A \(\push : \mathit{control} \times \mathit{Pkt} \to \mathit{control}\) offers the packet in parallel to every node on its root-to-leaf path, and each of those nodes enqueues the payload its own \zz returns, at the rank \zz mints for it.
Ranks are fixed at \push{} time and never revised.
A \(\pop : \mathit{control} \rightharpoonup \mathit{Pkt} \times \mathit{control}\) releases the best-ranked packet, resolving ``best-ranked'' from the top down: it releases the best-ranked index at the root, follows that index to a child, recurses, and releases a packet once it reaches a leaf.
...
Rio can compile any \disc that satisfies this interface.
...
Both denotations are defined by recursion on \(\pi\).
Here we focus on the base case for the empty path, which is where the work happens.
On the network operator's side, \Add{} appends an annotated arm to that node's child list:
{\small
\[
\psem{\Add([\,],\ D,\ \mathit{arm},\ m)}\ (D\ \mathit{ts}) \;=\; D(\mathit{ts}\mathbin{+\!\!+}[m{:}\,\mathit{arm}])
\]
}
Here \(\mathit{ts}\) is a node's list of annotated arms and \(\mathit{ts}[t/i]\) overwrites the policy of the \(i\)-th arm.
The step case descends to the on-path child and writes the result back, changing nothing else at the node it passes through.
...
\quad \phantom{\textbf{where}}\ z'=\mathtt{setmeta}(\mathtt{extend}(\mathit{z},\mathtt{flows}(\mathit{arm}),k),k,m)
\end{array}
\]
}
...
The step case is more involved.
While walking downward, it also applies \(\mathtt{extend}\) at every node it passes, sending the new flows one step further toward \(\pi\).
...
We write \(p \eqR p'\) when the two policies are identical up to the order of the arms at each node.
...
\(\lfloor C \rfloor\) is ``the \pol that \(C\) realizes.''
...
Compiling a policy and reading it back recovers the policy, up to the sibling order the compiler was free to choose when it laid it out: \(\lfloor \lceil p \rceil \rfloor \eqR p\).
...
\Cref{fig:tour} essentially drew two instances of this square side by side.
...
\Cref{sec:per-form} proves this correctness theorem, which suffices to show agreement in the above scenario:
%
...
We chose the six forms so that installing one never disturbs the backlog.
Installing \(\delta\) may change how traffic is served from that moment on, but the act of changing policies never drops, reorders, or re-ranks a packet that is already buffered, and each buffered entry keeps the rank it was given when it was \push{}ed.
...
A \pop{} reads only \pifo contents, never any \stt or \zz (\cref{sec:controls}), so most forms discharge the theorem by leaving every live \pifo entry untouched.
\Designate{} and \Undesignate{} disturb entries, inserting and dropping a node whose \pifo is not empty, and they are argued on the \pop{} sequence instead, which is why the theorem is stated on the flush sequence rather than on \pifo contents.
\cref{sec:per-form} proves this.

\subsection{Sequences and the Transition Planner}\label{sec:sequences}\label{sec:planner}
...
A guard \(\varphi\) is a condition on the live control that the substrate can be asked to wait for, and the only guard we need is that a given subtree has drained to empty.
A \emph{step} is one guard together with the run of \(\delta\)s that guard releases.
Both theorems hold at every step: each \(\delta\) fires on the well-formed control that the previous one left behind (\cref{thm:interplay}), so \(\lfloor \cdot \rfloor\) names the \pol in force at every point, exactly as \cref{sec:intro} promises of the transitional period, and the backlog survives the whole sequence (\cref{thm:nondisruption}).

An \emph{idiom} is just a named sequence.
\Retire(\pth) tears a subtree down gracefully: it quiesces the subtree at \pth, waits for it to drain, and then removes it.
\Replace(\pth, \survivor) swaps \survivor in for \(\mathit{pol}@\mathit{path}\): it designates \survivor beside the running subtree, quiesces that subtree, waits for it to drain, and then collapses the \(\Strictstar\) wrapper.

% Sequences are proposed rather than authored.
The operator hands the runtime \(p_1\), the presently running policy, and \(p_2\), the one they now want, and a \emph{transition planner} returns a guarded sequence carrying the one to the other, which the operator is free to accept or reject.
The planner is a heuristic, and we make no claim that what it emits is minimal.
It always reaches \(p_2\), because \(\Replace([\,], p_2)\) is available at every point and discharges the completeness criterion of \cref{sec:delta-grammar} on its own.
It reaches for that fallback only when none of its strategies finds a tighter edit, descending through the slots at which \(p_1\) and \(p_2\) already agree and installing the difference at the deepest slot it reaches, so that a change confined to one subtree leaves every other part of the tree scheduling its own traffic untouched.
The root-level fallback shows what that confinement is worth: it puts the entire running tree on the high-priority arm of a \(\Strictstar\), so every packet arriving during the transition waits behind the whole existing backlog.
...
Every step of the enqueue and dequeue walks of \cref{sec:controls} uses match--action tables~\cite{Bosshart14}, so installing a policy on such a substrate means writing entries into those tables.
...
Some instructions have preconditions, e.g., \texttt{emancipate} requires that the subtree it detaches is empty.
Enforcing them is the compiler's responsibility: it emits the whole sequence of transactions and guards, so an instruction that needs an empty subtree is preceded by a guard on that emptiness (\cref{sec:sequences}), and the controller holds the sequence there until the guard's predicate becomes true.
Rio does not guarantee progress, and the control plane must take care to wait only on subtrees that will eventually empty.
...
A PE stores four things: its \texttt{Assoc} set, the flows it admits; a \texttt{Map}, read on enqueue, naming the PIFO each admitted flow enters; a next-hop table, read on dequeue; and the designated-survivor link (\cref{sec:hw-survivor}).
%
Each of \zz's parts is the target of an opcode, so an edit that rewrites one of them lowers to instructions that write the corresponding PE state.
...
Instructions affect two kinds of state, match--action tables and ranking logic, and our key observation is that atomicity needs hardware support for the tables but \emph{not} for the ranking logic.
That suffices for all six edits, because each of them affects at most one \emph{live} scheduling policy and so can be staged.
\Add{} is the pattern they all follow: it first configures the policies for the new subtree and then activates it, and only the activation needs a transactional update, since the fresh subtree is not visible to live traffic until then.

Rio makes those table updates atomic by double-buffering, a common technique for atomically updating packet-processing pipelines~\cite{Xing22,Yang24}.
Configuration instructions write the inactive copy, and committing a transaction swaps the active and inactive copies.

A swap takes a single cycle, which makes updates atomic within a PE, but a transaction can affect several PEs.
...
Rio's second addition to the PIFO tree substrate supports the $\Designate$ and $\Undesignate$ policy edits (\cref{sec:delta-grammar}), which introduce and delete a $\Strictstar$ node.
A $\Strictstar$ behaves exactly as $\Strict$ does, and Rio implements it with a minimal hardware modification and without relocating packets across PEs.
The \texttt{designate} instruction creates a relationship between two PIFOs resembling a $\Strict$ node, but a restricted one: its priorities can never change, since \texttt{set\_arm\_meta} is disallowed, and only \texttt{undesignate} can delete it.

These restrictions admit a key optimization: to implement $\Strictstar(A, B)$ we need no separate PE for the $\Strictstar$ node itself, and instead add simple logic to the PE that already houses $A$ and $B$.
A dequeue request meets this survivor-link cell before it reaches the PE's PIFOs (\cref{fig:hardware}), and it fits into the dequeue pipeline without adding a cycle, whether or not the request is rewritten.
Adding and deleting a $\Strictstar$ node therefore shifts no tree levels, where shifting them would mean relocating $A$'s and $B$'s packets across physical PIFOs.

Rio adds to each PIFO a \emph{designated-survivor link}, initially null, and \texttt{designate} points the doomed node's link at the survivor's root.
...
The two must already sit in the same PE.
The parent is not told, so \texttt{undesignate} is later a single retarget that leaves the parent's per-arm metadata intact.

We modify the dequeue logic to follow these links: whenever a PE would dequeue from an empty PIFO, it checks for a link and dequeues from the survivor instead.
...
%
The guard of \cref{sec:sequences} gates PIFO reclamation too, since the doomed subtree's PIFOs may be released only once it is empty.
After it, nothing in the tree records that a transition happened.


\section{Evaluation}\label{sec:eval}

We implemented Rio's tree scheduler with two substrate additions in SpinalHDL, integrating on top of an existing PIFO implementation \cite{Sivaraman16,Sivaraman16Repo},
We implemented Rio's language and transition planner in OCaml.
We measured transitions in cycle-accurate simulation, and synthesized the design for an FPGA.
In total, the implementation contains 750 lines of Scala and 1500 lines of OCaml code.

\begin{figure}[t]
\centering
\includegraphics{fig-eval-motivating.pdf}
\caption{The transition of \cref{sec:tour}, as a share of link capacity. The lower panel is just \textsf{zoom}, which the transition request ought not affect. $p_\text{temp}$ is in effect during the gray band.}
\label{fig:eval-motivating}
\end{figure}

\paragraph{The baseline}

A switch that stops to reconfigure can do many things with the traffic that arrives when it is stopped.
Rather than try to try and measure that chaos, we remove it from the comparison.
...
%
We also give it a short stoppage of 1024 cycles, a little over a microsecond at one nanosecond per cycle, which is far shorter than any published figure: reconfiguring the program a running switch executes has been reported at around half a second~\cite{Xing22}.

\paragraph{The transition of \cref{sec:tour}}

Our first experiment considers exactly the transition that \cref{sec:tour} presents, taking the scheduler from \(p_1\) to \(p_2\) over one 2136-packet trace by three mechanisms.
\emph{Stop the world} is as above.
...
\emph{Confined replace} is the path the planner prefers, which does the same thing at the deepest slot that changed.
...
Before the request, the three together ask for more than the link can carry, so a backlog is standing when the request arrives at cycle 0.
...
\Cref{fig:eval-motivating} shows each mechanism's share of the link and \cref{fig:eval-delay} shows how long a packet waits between arrival and departure, and together they separate two ways a transition can go wrong.
Stopping the scheduler empties the link, so throughput drops to zero for as long as the stop lasts.
Root-level replace keeps the link busy and clears the \textsf{gmail} backlog quickly, but the \textsf{zoom} panels reveal what it is busy with: it hands the whole link to the old tree, starving \textsf{zoom} for some 300 cycles before letting it catch up.

\begin{figure}[t]
\centering
\includegraphics{fig-eval-delay.pdf}
\caption{Delay from arrival to departure, per packet. The upper panel is \textsf{zoom} and the lower is \textsf{gmail}.}
\label{fig:eval-delay}
\end{figure}

Any delay that \textsf{zoom} suffers is the transition's doing.
Only Rio keeps it steady (\cref{tab:eval-motivating}): its peak delay is within a cycle of what \textsf{zoom} sees under a transition that never touches its subtree, where the other two cost it an order of magnitude more. Neither recovers when its interval ends.
% Under confinement, \textsf{gmail}\textsubscript{old} takes its own 670-cycle drain and no more.

The gray band in both figures is the interval in which the switch runs \(p_\text{temp}\), the transitional policy the planner discovered on the way to \(p_2\).
Its two edges are the two atomic steps of \cref{sec:tour}.
The band is two thirds as wide as the baseline's stop and, unlike that stop, costs the traffic nothing: the link stays full and \textsf{zoom} is undisturbed from one end of it to the other.
...
Any other way of doing this must put the backlog inside a newly built node.
Our second experiment measures the same transition across backlogs from 0 to 341 packets standing at the request, by Rio and by each of the two ways of building such a node.
Reserving a spare processing element and filling it with a token per buffered packet stops the scheduler for 0.991 cycles per packet, and copying the old subtree onto fresh processing elements stops it for 2.969.
...
Our third experiment holds the request fixed and grows the tree, which runs \WFQ{} over \(m\) tenants, each a two-flow subtree with its own discipline, with \(m\) dialled up from 2 to 32 so the tree holds 4 to 64 flows.
Offered load stays at 1.10 of the link at every point and is spread evenly over the flows, so the congestion does not change.
The transition request in each case is to \(\Add\) a tenant at the root.

Rio issues the same 21 instructions at every point of the sweep and finishes within a cycle of the same time throughout (\cref{fig:eval-scaling}), and it never stops the scheduler.
Whole-tree prefill and stop-the-world reset both grow by more than an order of magnitude across the same range, because they reinstall every tenant whether the request named it or not.
...
The lower panel measures a flow that the request does not touch.
Rio costs it nothing on the smaller trees and a few tens of cycles on the largest, where prefill costs it hundreds throughout and climbs steadily.
Stop-the-world is dominated by the 1024-cycle stop we grant it until the largest tree, where the cost of rebuilding finally overtakes even that.

\paragraph{A larger reconfiguration}

For our fourth experiment, we set up a fourteen-flow tree on seven processing elements, and we change its policy to reweight two tenants, admit four more, and retire one.
...
\bottomrule
\end{tabular}
\caption{Resource overhead on a Xilinx XCKU5P, using five processing elements with 1024 PIFO entries each. Each figure is a percentage of the same design without Rio additions.}
\label{tab:eval-resources}
\end{table}
% \xxx[zhiyuan]{for the table we can also note percentage of hardare resources on FPGA}

We synthesize the PIFO engine, both with and without Rio's additions, targeting a Xilinx XCKU5P FPGA and sweeping the number of virtual flows addressable by the tables (\cref{tab:eval-resources}).
The added logic meets timing at $250\,\mathrm{MHz}$, as the baseline design does, and the PIFO itself is unchanged and continues to operate at $100\,\mathrm{MHz}$.
Rio's logic overhead stays under a tenth of the baseline's LUTs and flip-flops across a 32-fold range in flow count.
Memory is where the additions are paid for, and only once the tables are large: the widest configuration needs 7 additional BRAMs, bringing the augmented design to at most 5.9\% of the FPGA's available BRAMs.
That overhead stems from the double-buffered tables of \cref{sec:hw-commit}, since making an update atomic means holding a second copy of the state.
...
FlexCore~\cite{Xing22} reconfigures the P4 program running on a switch.
It computes a reconfiguration script, a sequence of atomic graph edits that transforms the old program into the new one.
It offers three consistency guarantees that govern the intermediate states visible between the two endpoints.
Rio generates a series of atomic edits to transform a scheduling policy into another (\cref{sec:sequences}).
...
FlexCore's strongest guarantee, \emph{program consistency}, lets no intermediate state reach a packet; a Rio transaction provides that for a single \(\delta\) (\cref{sec:hw-commit}).
...
Rio's focus on packet schedulers, as opposed to forwarding pipelines, leads to different techniques, and the difference is state.
A scheduler holds in-flight packets in its PIFOs and per-policy bookkeeping at every node, where FlexCore does not support stateful programs at all, so Rio has the additional challenge of preserving the state reflecting past packets while reconfiguring the policy for future ones.
...
SP-PIFO~\cite{Alcoz20} changes the mapping from packet ranks to a fixed set of strict-priority queues as traffic shifts, minimizing the error with respect to an ideal PIFO (the policy that the operator wrote).
Rio focuses on changing the policy itself.

\citet{Alcoz23} consider scheduler reconfiguration.
Their scheduling hypervisor deploys per-tenant scheduling policies onto a switch, and they ask how to move from one such policy to another as tenants come and go.
They name two obstacles: emptying the buffers and resetting the state of the scheduling functions, and leave the mechanism to future programmable data planes.
Rio answers these by draining rather than dropping packets (\cref{sec:quiesce}) and by declining to reset (\cref{sec:changemeta}).
...
FairNIC gives each tenant an independent subtree of packet schedulers to configure~\cite{Grant20}.
...
Subsequent work generalizes the primitive, approximates it on hardware that ships today, and scales it to large flow counts~\cite{Shrivastav19,Alcoz20,Alcoz25,Mostafaei25,Yao23,Atre24,Gao22}.
...
Reconfiguring a packet scheduler inevitably entails some period
...
We used ChatGPT to construct experiment scripts, and Claude Code to analyze results. 
Claude was also used to convert a preliminary draft of this work from Markdown to \LaTeX.
Authors are fully responsible for all content written, and authors confirm that the content is, to the best of their knowledge, accurate and correct.
...
We address node-local components of a control by a \pth: the local triple reached by following \pth from \(C\)'s root is written \(C@\mathit{path}\), with fields \(C@\mathit{path}.\mathit{state}\), \(C@\mathit{path}.\mathit{pifo}\) and \(C@\mathit{path}.\mathit{z}\), and we write \(C@\mathit{path}.\mathit{node\_state}\) and \(C@\mathit{path}.\mathit{slot\_states}\) for the two components of \(C@\mathit{path}.\mathit{state}\).

\paragraph{The parts of \zz}

\Cref{sec:controls} names \zz's four parts, and here they are in full.
The \emph{classifier} answers whether this node accepts a given packet and where it sends it.
It has a \emph{service map}, written \(\mathit{z}.\mathit{serves}\), which is a partial map from flows to payloads.
At an internal node, \(\mathit{z}.\mathit{serves}(f)\) is a unique child index, following which brings a packet of flow \(f\) one step closer to its leaf.
This map is easy to construct since distinct leaves carry distinct flows (\cref{sec:policy-dsl}), and at a leaf the payload is the packet itself, so the map is the trivial one on the single flow that leaf carries.
...
The \emph{per-arm metadata}, written \(\mathit{z}.\mathit{meta}\), records what this node's discipline needs to know about each of its arms, such as priorities or weights; a leaf has no arms, and so no metadata.
The \emph{ranker}, written \(\mathit{z}.\mathit{rank}\), is the scheduling logic that the node's discipline contributes.
Its inputs are the node's \stt, the chosen payload, and that payload's arm metadatum, and from these it mints the rank and the updated \stt.
...
At an internal node the ranker sees the payload and not the packet, so the node can discriminate only among its own children, and its behavior is a function of its own arms alone.
Mohan et al.\ keep one total transaction for the whole tree instead, returning an entire root-to-leaf path of indices and ranks, where ours is per-node and partial.
...
Appending the arm at the base case is legitimate because our arm-order freedom (\cref{sec:equivalences}) lets us place the new child last.
...
Inverting the two priorities of \(\Strict(\mathtt{hi}{:}\,A,\ \mathtt{lo}{:}\,B)\) does \emph{not} change the next \pop{}, since the form does not touch the parent's index-\pifo.
...
\Quiesce{} strikes every flow that \(C\) routes into \(C@\tau\) from the admitted sets of the nodes that flow traverses, so that no packet the tree would deliver there is admitted anywhere, while leaving every node's routing in place so buffered packets can still be popped out.
...
The flows to silence are the ones the control would carry into \(C@\tau\).
Write \(\mathtt{route}(C, f)\) for the nodes a packet of flow \(f\) visits: start at \(C\)'s root and follow \(\mathit{z}.\mathit{serves}(f)\) at each internal node until a leaf is reached.
Each \(\mathit{z}.\mathit{serves}\) sends a flow to a single child, so the route is unique, and
\[
L = \{\, f \;:\; C@\tau \in \mathtt{route}(C, f) \,\}.
\]
Wherever \(C\)'s leaves carry pairwise distinct flows this is \(\mathtt{flows}(C@\tau)\), the flows that \(C@\tau\) serves; the note below covers the one case in which the two part company.
...
Each route into \(C@\tau\) runs down the spine to \(\tau\) and continues inside it, so the nodes to narrow lie in
\[
...
Let \(L = \mathtt{flows}(C@\tau)\) be the flows that the doomed arm serves.
...
It is defined when \(\tau\) resolves in \(C\), when the subtree at \(\tau\) is empty, and when neither \(C@\tau\)'s ranker nor \(C@\pi\)'s names \(\Strictstar\).
...
The same holds of a wrapper's arms, since \Remove{} retracts \(L\) at the parent and above, which at a wrapper would strike the flows \Designate{} routed to the survivor.
...
But \emph{which} two?
No entry in \(P\) carries the meta-information ``I was enqueued when a packet was inserted into \tritext{flowC}''; an entry \(0\) in \(P\) only means ``when this index is popped, recursively ask subtree \(Q\) to emit the next packet'' (see \cref{sec:controls}).
...
A non-empty \(\tau\) destructs as \(\pi \mathbin{+\!\!+} [k]\), as before.
...
\quad q_N = \text{the \pifo of } P_0 \text{ entries, each the payload } 0\\
\quad\qquad\qquad \text{at the rank } \mathit{rank}_{\Strictstar} \text{ mints for } \mathtt{hi},\\[3pt]
\quad \mathit{z}_N.\mathit{serves}(f) = 1\ \text{if}\ f \in \mathtt{flows}(\mathit{survivor}),\ \text{else}\ 0,\\
\quad\qquad \text{on } \mathrm{dom}(\mathit{z}_N.\mathit{serves}) = \mathtt{flows}(M) \cup \mathtt{flows}(\mathit{survivor}),\\[3pt]
\quad \mathit{z}_N.\mathit{admit} = \mathrm{dom}(\mathit{z}_N.\mathit{serves}),\\[3pt]
\quad \mathit{z}_N.\mathit{meta} = \{\, 0 \mapsto \mathtt{hi},\ 1 \mapsto \mathtt{lo} \,\},\\[3pt]
\quad \mathit{z}_N.\mathit{rank} = \mathit{rank}_{\Strictstar}.
\end{array}
\]
}
...
Below we write \(N\) for that node and \(B\) for its surviving arm, both as controls, and \(|B|\) for the number of packets buffered under \(B\), with \(\pi\) and \(k\) as in \cref{sec:designate}.
...
This includes the controls that no compilation produces: one carrying a standing backlog, or one that a \(\delta\) has since edited.
Its agreement with \(\lceil \cdot \rceil\) is therefore something to prove.
...
Outside the region \cref{sec:edit-semantics-full} specifies, \(\csem{\delta}\) is undefined.
...
\item \emph{Characterization.} Prove that reading the policy off the edited control agrees, up to \(\eqR\), with applying the \(\psem{\delta}\) of \cref{sec:edit-semantics-full} to the policy read off the original: \(p' \eqR \psem{\delta}(p)\).
...
A \pop{} consults three things about a live entry: its rank, its place in the queue, and the child it names.
An entry is undisturbed when all three survive, whatever numeral denotes that child.
So if \(\csem{\delta}\) leaves every live entry of \(C\) undisturbed, never reordering or removing one, and introduces only empty \pifo{}s, then the backlog survives entry-for-entry and \(\flush(C) = \flush(C')\).
...
Fifteen of the eighteen checks are immediate from the rules of \cref{sec:edit-semantics-full}: they name the fields that a rule writes and observe that the property at issue does not depend on them.
...
\paragraph{Non-disruption}
\(\mathtt{drop}_k\) compacts the child list and relabels the entries above the gap in the same step, so each one goes on reaching the child it reached before, at the rank and in the place it already held.
Emptiness leaves no entry equal to \(k\) for it to delete, and the ancestor retractions delete nothing either, since they narrow admission for future pushes only.
The substrate does the same thing without relabeling anything, since it leaves slot indices where they are (\cref{app:lowering}).

\subsection{\texorpdfstring{\Designate}{Designate}}\label{sec:check-designate}

Throughout this subsection and the next, \(N\) is the wrapper node, \(B\) the control under its arm 1, and \(\pi\) and \(k\) the parent path and slot, as in \cref{sec:designate}.
...
Because \(\eqR\) (\cref{sec:equivalences}) only permutes sibling arms, each \(\eqR\)-class has a canonical representative under any fixed ordering of the arms at a node, and the normalization below picks it.
...
If the roots agree, the task reduces to \emph{pairing up the two child lists}, and the comparator settles each pairing on its own.
...
Neither positional strategy applies: the lengths differ, and the sub-sequence check fails because \textsf{gmail} in \(p_1\) equals no arm of \(p_2\), so without the label-overlap walk the planner would emit a \Replace{} of the whole root \(P\).
...
The comparator emits three: an \Add{} of \textsf{slack}, a \ChangeMeta{} re-tuning \textsf{gmail}'s slot to \(m_4\), and a \Replace{} at that slot.
Those three make two steps (\cref{sec:sequences}), since the only guard is the one inside \Replace{} that waits for the old \textsf{gmail} leaf to drain.
...
In the settled tree \textsf{slack} sits beside the rebuilt \textsf{gmail} subtree in an order that does not match \(p_2\) literally, but sibling order carries no semantics (\cref{sec:equivalences}), so the reordering is invisible to any observer.
...
\cref{fig:lowering} lowers all six forms of \(\delta\) to the ISA of \cref{sec:hw-isa}, in the order \cref{fig:delta-grammar} lists them, and \cref{sec:hw-commit} walks through the \Add{} case in prose.
...
Of the queries, \texttt{flows(v)} is the set of flows served beneath \texttt{v}, less any that \texttt{v}'s designated survivor has taken over, and on flow sets \texttt{\&} is intersection and \texttt{\textbackslash} difference.
Admission does not enter it, so a \Quiesce{} leaves it alone and the walks that fire after a drain still have flows to walk.
...
\texttt{survivor(v)} is \texttt{v}'s designated survivor, and \texttt{freeslot(v)} is an index that no child of \texttt{v} holds.
...
\Add{}'s new subtree takes a slot that no child of \texttt{v} holds, so existing children's indices are undisturbed, and its leaf-disjointness precondition (\cref{sec:add}) means no PIFO on a chain already routes \texttt{f}, so neither walk overwrites a live entry.
\Remove{} is \Add{} run backwards.
The \(\sigma_k\) renumbering of \cref{sec:remove} has no counterpart on the substrate: a child sits at the index \texttt{adopt} named for it, and \texttt{emancipate} frees that index without moving another, so every \texttt{map} and \texttt{set\_arm\_meta} entry naming a surviving sibling stays valid.
Slot indices are stable and need not be dense, where \cref{sec:remove} keeps a dense child list and renumbers to restore it, and the two agree on the child that each surviving entry reaches.

\Quiesce's wide reach, covering the root-to-\pth chain along with the whole subtree under \texttt{v}, is forced by \cref{sec:controls}'s parallel \push{}: every node on a packet's path mints its own routing index independently, so dropping \texttt{f} at only some of them would leave the rest minting stray indices, and the tree malformed.
...
% Flush the deferred float before acmart's end-of-document \balance runs;
% the balance package drops floats that are still deferred when it fires.
\clearpage
