...
We show how to extend a PIFO-tree-based packet scheduler with support for live policy reconfiguration.
Our design starts with a standard PIFO tree architecture (\cref{sec:hw-substrate})
and adds two key mechanisms:
atomic state updates (\cref{sec:hw-commit})
and support for $\Designate$ (\cref{sec:hw-survivor}).
We define an ISA for this reconfiguration and demonstrate a compiler that targets it (\cref{sec:hw-isa,sec:hw-compile}).
\xxx[as]{revisit: should ISA go first? also, hopefully \cref{sec:substrate-portability} collapses}
