...
Rio's second addition to the PIFO tree substrate supports the $\Designate$ and $\Undesignate$ policy edits (\cref{sec:edit-dsl}).
...
We show how to support $\Strictstar$ with a minimal hardware modification and without relocating packets across PEs.
...
\xxx{In other words, to calculate the level of a given node, ignore strictstar...}
...
\xxx[as]{Would it be correct to just say (instead of the previous sentence): as a precondition to designate(v, surv), v and surv must already be in the same PE.}
...
\xxx[as]{Is that true? There was previously a longer description of what happens on enqueue, but the way I read it, it boiled down to ``nothing needs to change'': you just broadcast the packet to all the PEs as usual, and the PEs as usual know which PIFOs to push into. So I compressed to this one sentence...}

\xxx[as]{Perhaps this paragraph is deletable as well?}
In effect, the resulting $\Strictstar$ node is ``transparent'' to the rest of the tree above it.
The parent PIFO contains one entry per packet in \emph{either} subtree.
...
\xxx[as]{I added the previous sentence, because I think it is a more direct summary of what we were saying. But if that's the case, readers will now wonder why we have to wait for a pop to happen later (why there are two steps:: ``arm'' and ``enable,'' per the next sentence). To keep things simple (even if it is an oversimplification of what the hardware implements), can we just say that the effect happens immediately, because the subtree is already empty? It would simplify the story a lot, i.e., this could just be one sentence.}
