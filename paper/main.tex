...
Its tree shape, its per-node disciplines, and its table contents are all artifacts of a decision made before the switch was brought up~\cite{Sivaraman16,Gao22,vPIFO}.
The compilers that produce them accept \emph{a policy to run} rather than \emph{a change to the policy already running}~\cite{Sivaraman16,vPIFO}.
So a PIFO tree that is already running has no way to reach a new policy but to stop the world: drop or recirculate the buffered packets, recompile, and reinstall~\cite{vPIFO,Alcoz23}.
