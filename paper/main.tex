...
\xxx[as]{anything else?}
Each level in the policy tree (i.e., the set of nodes at each tree depth) maps to a PE.
Because each level must track ranks for all in-flight packets, all PEs use an equal amount of total PIFO capacity,
divided among the nodes at that layer.

To enqueue a packet, the architecture finds the set of PIFOs to insert into (by classifying the packet) and computes the rank for each according to its policy.
We assume the hardware comes with rank-assignment logic to support a set of policy disciplines, $D$.
