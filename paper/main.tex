...
Our controller accepts a transaction's rank-computing writes along with the rest of its instructions, and applies them as it reaches them rather than holding them until the transaction installs.
...
\texttt{v}'s PIFOs, and those of everything beneath it, may be released only once the subtree is empty, and a second transaction is guarded on the same emptiness predicate (\cref{sec:hw-commit}).
...
Rio reaches per-packet consistency by another means: a transaction installs in a single cycle at each PE, so a packet meets one configuration or the other (\cref{sec:hw-commit}).
...
The strongest of their guarantees, program consistency, lets no intermediate state reach a packet, and a Rio transaction provides that for a single \(\delta\) (\cref{sec:delta-grammar}).
...
A Rio transaction is a group of such entry writes that installs together, so that the tables that a packet consults along its path never disagree about which policy is in force (\cref{sec:hw-commit}).
...
Rio runs above any of them and asks for one property: transactions that are atomic with respect to traffic (\cref{sec:hw-commit}).
