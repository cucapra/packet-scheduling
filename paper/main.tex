...
\caption{The transition of \cref{sec:tour}, as a share of link capacity. The middle panel is just \textsf{zoom}, which the transition request ought not affect; the lower one is all three flows under Rio. The gray band is the interval in which Rio runs $p_\text{temp}$.}
...
The second, \emph{whole-tree replace}, builds \(p_2\) fresh alongside the running tree and drains the old one behind it.
The third is Rio's \emph{confined replace}, which installs the \(\Strictstar\) transitional policy and collapses it once \textsf{gmail}\textsubscript{old} is empty.
Throughout, \textsf{zoom} offers 0.40 of the link, \textsf{gmail} offers 0.80 before the request and 0.20 after it, and \textsf{spotify} arrives with the request offering 0.20.
...
\bottomrule
\end{tabular}
\caption{The transition of \cref{sec:tour}, three ways.}
\label{tab:eval-motivating}
\end{table}

\Cref{fig:eval-motivating} separates two ways a transition can go wrong.
Stopping the scheduler empties the link, so the orange line in the upper panel drops to zero for as long as the stop lasts.
Whole-tree replace looks better in that panel, since it keeps the link busy, but the middle panel reveals what it is busy with: it starves \textsf{zoom} for some 500 cycles and then hands it the whole link to catch up.
We know that \textsf{zoom} sits at the same priority and offers the same rate in every policy here, so a mechanism that touches only what changed should leave textsf{zoom}'s line flat, and only Rio's is flat.
Rio's peak delay for \textsf{zoom} is 21 cycles, one more than the 20 it sees under a transition that never touches its subtree at all.
Whole-tree replace costs it 613 and the baseline 1055 (\cref{tab:eval-motivating}), and neither of them recovers when its interval ends: the baseline is still running above its offered rate 3400 cycles after a stop that lasted 1024 cycles.

Rio's own transitional interval is no shorter than the others.
The gray band in the figure is the 1018 cycles in which the switch runs \(p_\text{temp}\), a policy no operator requested, and it costs the traffic nothing: the link stays full and \textsf{zoom} is undisturbed from one end of it to the other.
The lower panel shows all three flows.
...
Happy to move it to the appendix and drop the prose about it, if \S8 needs the space.}
At 21 cycles, confinement is also the cheapest of the three to install, since it rewrites one subtree and leaves the rest of the control addressed as it was.
...
Our second experiment measures the same transition three ways, across backlogs from 0 to 507 packets standing at the request.
Materializing the wrapper on a reserved element costs 0.990 stopped cycles per buffered packet, and copying the subtree onto fresh elements costs 2.975.
A survivor link costs zero at every point of the sweep, because it is a pointer rather than a place to put packets.
The slope matters more than the constant: without \(\Strictstar\), the price of a transition is set by how busy the switch was when the request arrived, which is when an operator can least afford to pay it.
This is also why \textsf{spotify} waits in \cref{fig:eval-motivating}, since Rio declines to relocate the packets already queued and lets them drain in place instead.

\paragraph{A larger reconfiguration}

Our third experiment considers how Rio scales.
...
Rio's instruction count tracks the part of the tree the request names, where both baselines rebuild the tree entirely.
Rio does not stop the scheduler at all, and the two baselines buffer 624 and 495 packets during theirs, which they can do only because we gave them unbounded input queues.
