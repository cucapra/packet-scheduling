...
\xxx[as]{I could use some help understanding this part. I think it's saying that the ``brain'' updates are non-transactional. If that's the case, we need a bit more of an argument for why this is OK. If a transaction includes both topology changes and discipline/metadata changes, how do we guarantee that the whole thing applies atomically? (``That's not allowed in the ISA'' is an acceptable answer.)}
Also, the rank computation must accept incremental updates rather than wholesale replacement of a live scheduling policy.
...
\xxx[as]{Please correct me if I'm wrong, but I think the previous sentence is saying: instructions like \texttt{set\_policy} are applied immediately, instead of waiting for the transaction to commit.}
