...
With these components in hand, a scheduler can clearly define its behavior during reconfiguration.
We present \emph{Rio}, which supplies all three.

Rio defines a grammar of atomic \emph{policy edits} and designs an extension to PIFO tree hardware to support these edits.
...
we give formal semantics for PIFO trees and Rio's edits.
...
correspond to equivalent effects on the high-level policy definition.

We implement a simple PIFO tree packet scheduler and demonstrate two extensions to support atomic policy edits:
transactional state updates and a mechanism for letting old and new subtrees coexist during a transaction.
\xxx[as]{Short preview of the results.}
