...
With Rio's additions, as in the baseline design, the added logic meets timing at $250\,\mathrm{MHz}$. The PIFO itself is unchanged and continues to operate at $100\,\mathrm{MHz}$.
Rio's logic overhead is low: it uses at most 3.2\% more LUTs and 8.8\% more registers across a 32-fold range in flow count.
For memory, 7 additional BRAMS (or 35\% more than the baseline) are required.
In total, the augmented design consumes at most 5.9\% of FPGA's available BRAMs.
Its memory overhead mostly stems from the double-buffered tables
(\cref{sec:hw-commit}): making an update atomic means holding a second copy of the state.
