...
\citet{Reitblatt12} uses a versioning scheme:
...
Rio shares the goal of offering each packet a consistent view of configuration state but focuses on a single packet scheduler.
...
Rio similarly works by generating a series of atomic edits to transform one packet-scheduling policy into another (\cref{sec:delta-grammar}).
Every intermediate state during a Rio update is a legible program in the same language that the user writes to specify policies (\cref{sec:policy-dsl}).
Its guarantees at the per-edit level therefore resemble FlexCore's strongest consistency level, \emph{program consistency}.
...
Rio's focus on packet schedulers, as opposed to programmable switches, leads to different techniques.
