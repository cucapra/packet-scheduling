...
The other two are both Rio's, and the difference between them is where the \(\Replace\) lands.
...
Root-level replace looks better in that panel, since it keeps the link busy, but the middle panel reveals what it is busy with: it starves \textsf{zoom} for some 500 cycles and then hands it the whole link to catch up.
...
Root-level replace costs it 613 and the baseline 1055 (\cref{tab:eval-motivating}), and neither of them recovers when its interval ends: the baseline is still running above its offered rate 3400 cycles after a stop that lasted 1024 cycles.
...
Happy to move it to the appendix and drop the prose about it, if \S8 needs the space. I just thought it was fun.}
...
One reserves a spare processing element and fills it with a token for each buffered packet, which stops the scheduler for 0.990 cycles per packet.
The other copies the old subtree onto fresh processing elements, which stops it for 2.975 cycles per packet.
