...
We identify a small set of \emph{policy edits} that PIFO tree hardware, with some minor extensions, can support atomically:
all packets before the edit see the old scheduling policy,
and all packets after the edit see the new scheduling policy.
This set of edits is also complete:
for any pair of policies, a chain of edits exists to transform between them.
We define a formal semantics for PIFO trees and their edits, and we use it to prove that our edits consistently implement policy changes on live PIFO state without disrupting in-flight traffic.
We show how to extend a PIFO tree architecture with two mechanisms to support policy edits:
atomic state updates and low-cost prioritization between old and new policy subtrees.
Hardware simulation experiments demonstrate that the system keeps reconfiguration latency low and, during reconfiguration, disrupts traffic less than a stop-the-world approach to reconfiguration.
\end{abstract}
...
\Cref{sec:eval} evaluates this transition.
...
We know that \textsf{zoom} sits at the same priority and offers the same rate in every policy here, so a mechanism that touches only what changed should leave \textsf{zoom}'s line flat, and only Rio's is flat.
...
\xxx[zhiyuan]{can we in figure to show that we follows the theory?}
...
\xxx[zhiyuan]{maybe atlease include a scatter figure?}
...
\xxx[zhiyuan]{additionally, find a place to show one example of four operations: start, commit, replay-finish, drain, maybe a full timeline under figure 7 include the cleanup commit.}

\paragraph{A larger reconfiguration}

\xxx[zhiyuan]{here we can insert some bar figures (comparing the different trees) mainly targets to show "we can work under different scales of the tree, and different traffic pattern", people will be interested in if the method is scalable? so comparing cycles/latency/etc. under different reconfig scale is always a good thing. as we have multiple transitions. I can also do different packet distribution}.
...
\begin{tabular}{rrrr}
\toprule
& \multicolumn{3}{c}{Rio's Additions compared to Orignial PIFO} \\
\cmidrule(l){2-4}
