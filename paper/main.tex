...
We synthesized the PIFO Engine, both with and without Rio's additions, targeting a Xilinx XCKU5P mid-range FPGA and sweeping the number of virtual flows addressable by the tables (\cref{tab:eval-resources}).
...
Rios adds little resource overhead, mostly for the second table bank of \cref{sec:hw-commit}: making an update atomic means holding a second copy of what the update rewrites.
The additions cost little logic: at most 3.2\% more LUTs and 8.8\% more registers across a thirty-two-fold range in flow count.
For memory, 7 additional or 35\% more block RAM is needed, in total consuming at most 5.9\% of board's available block RAM resource.
