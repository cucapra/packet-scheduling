...
The request arrives at cycle 0.
...
\textsf{zoom} sits at the same priority and sends the same rate of traffic under all three, so any delay it suffers is the transition's doing.
...
\cref{fig:eval-delay} shows how long a packet waits between arrival and departure.
Root-level replace again looks good in the \textsf{gmail} panel, but the \textsf{zoom} panel reveals the problem: it clears the backlog quickly because it hands the whole link to the old tree, at \textsf{zoom}'s expense.
...
The first step installs the \(\Strictstar\) node above \textsf{gmail}\textsubscript{old} and the freshly built \RR{} subtree, and it is the cheapest install of the three (\cref{tab:eval-motivating}) because it rewrites one subtree and leaves the rest of the control addressed as it was.
The second collapses that node once \textsf{gmail}\textsubscript{old} has drained and the guard of \cref{sec:sequences} becomes true.
...
Any other way of doing this must put the backlog inside a newly built node, which means writing one scheduler token per buffered packet while the scheduler is stopped.
...
FlexCore~\cite{Xing22} consistently reconfigures the P4 program running on a switch.
