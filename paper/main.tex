...
\xxx[as]{This is what remains of the old compiler section.}
Each edit (\cref{sec:delta-grammar}) compiles to one transaction.
\cref{app:lowering} details the translations.
...
A \texttt{change\_arity} raising \texttt{v}'s count to \texttt{n+1} then creates the slot for the new subtree, \texttt{set\_arm\_meta} adds the relevant metadatum, and an \texttt{adopt} attaches the new subtree.
%
Traffic reaches the new arm last, by the lowering's counterpart of \(\mathtt{extend}\) in \cref{sec:edit-semantics}.
\xxx[as]{I don't understand the previous sentence. What does it mean to reach an arm ``last?''}
