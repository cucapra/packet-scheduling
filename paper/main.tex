...
Our thesis is that the interval during which a scheduler is being reprogrammed is itself a scheduling policy, and that understanding it is a language problem.
A transitional policy needs a syntax that the operator can read, a semantics that says which packet leaves next, and a compiler that accepts a change to the running policy.
With those in hand, whether a transition was carried out correctly becomes a theorem about the semantics rather than a measurement taken after the fact.
We present \emph{Rio}, which supplies all three.

Rio's language is a grammar of atomic policy edits, where each production is a change the hardware can install in a single instant.
Its semantics gives every edit a meaning in the operator's own policy language, so the scheduler is always running a policy the operator can read, even mid-transition.
A planner turns a requested policy into a sequence of those edits, and a mechanical lowering carries each edit down to the instructions that a switch runs.
A transition from any running policy to any requested policy is always available, and many touch only the part of the tree that actually changes, carrying the rest over unchanged.
What becomes of the backlog is no longer up to the implementation, and we call the property that settles it \emph{non-disruption}.
Reconfiguring never drops, reorders, or re-ranks a packet that is already buffered, so any disruption the operator observes is the new policy correctly governing new arrivals.
\xxx[AM]{Conclude here once the results are in. The cost model is still a legit open question, and this is where we say what we shorten and what we stop dropping.}
