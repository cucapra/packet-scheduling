...
But the scheduler was compiled expressly for the policy it is running: its tree shape, per-node disciplines, and table contents are all artifacts of a decision made with \emph{a fresh policy} as the goal, not \emph{a change to the running policy}~\cite{Sivaraman16,Gao22,vPIFO}.
Say the operator requests a new policy.
% A compiler that produces such a scheduler accepts \emph{a policy to run} rather than \emph{a change to the policy already running}~\cite{Sivaraman16,vPIFO}.
The freshly compiled scheduler knows nothing about the running policy or the backlog of buffered packets~\cite{vPIFO,Alcoz23}, and so the whole tree is rebuilt, including the nodes that could have been carried over unchanged.
What becomes of the backlog is left to the deployment: it can be dropped, recirculated, or left to be scheduled through a chaotic period in which the hardware follows no clearly defined policy.
Each of those is itself tacitly a \emph{scheduling decision}, and the network operator is unable to specify which one they want.

This paper writes that decision down.
Some changes can be installed as a single atomic step, and others cannot, leaving the scheduler to run for a while under a policy that no operator requested.
That interval is unavoidable and it persists for nonzero time.
The scheduler goes on accepting and emitting packets throughout, so scheduling decisions are being made by it.
...
Its semantics gives every edit a meaning in the operator's own policy language, so \emph{any transitional policy the grammar produces is ordinary scheduling}, readable exactly as the operator's own policies are (\cref{sec:edit-semantics}).
A planner turns a requested policy into a guarded sequence of those edits (\cref{sec:planner}), and a mechanical lowering carries each edit down to the instructions that a switch runs (\cref{sec:hw-compile}).
A transition from any running policy to any requested policy is always available, and many admit a realization confined precisely to the part of the tree that actually changes.
We give the semantics of the stop-the-world transitional policy and use it as a baseline.
We beat it with a property we call \emph{non-disruption}.
The act of reconfiguring never drops, reorders, or re-ranks a packet that is already buffered, so any disruption the operator observes is the new policy correctly governing new arrivals (\cref{sec:nondisruption}).
Two practical goals guide the search for better ways to transition between policies.
...
\item They are \emph{non-disruptive}. Installing an edit never drops, reorders, or re-ranks a packet that is already buffered (\cref{sec:nondisruption}). Edits that delete a subtree, such as $\Remove$, have a precondition that requires the doomed subtree to be empty.
\end{enumerate}
%
