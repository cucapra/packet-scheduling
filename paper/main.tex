...
\xxx[zhiyuan to am/as]{I'd like to insert have a hardware figure here; still WIP; let me know what you think we can include in the figure and I can make the edit.}
...
\begin{figure}
  \centering
  \includegraphics[width=0.8\columnwidth]{pifo_hardware.pdf}
  \caption{Rio PFIO Hardware. Adding transactional commit and support for $\Designate$ on top of PIFO implementations.}
  \label{fig:hardware}
\end{figure}
...
\xxx[am]{Notation collision. \(D\) is a single discipline everywhere else in the paper, as in \(M_{D}\), \(\mathit{rank}_{D}\), and \Add{}'s second argument.
Here it names the whole menu.}
\xxx[as]{No, I meant it the same way. Delete ``set of'' if you think that makes it clearer.}
%
...
\xxx[am]{Eek, collision!
\emph{Transaction} is already used for \zz: \cref{sec:controls} calls \zz the node's \emph{scheduling transaction}, and \cref{sec:tour} and \cref{sec:edit-semantics-full} use it that way throughout.}
\xxx[as]{I think this is fine. ``Transaction'' is a common word; the reader will be able to understand the difference. I will argue for keeping this thing named ``transaction'' because any other name will sound weird to knowledgable readers.}
...
\xxx[AM to Zhiyuan]{This sentence (from Adrian) feels slightly wrong to me.
The emptiness preconditions cannot be checked statically, so I do not see how a compiler enforces them; I believe \cref{sec:sequences} discharges them at runtime, with a guard that the controller waits on.
Please chime in and say which it is on your side.}
\xxx[as]{Here is how the compiler checks this. It executes a guard saying to wait for subtree A to become empty, followed by a transaction to remove A. Therefore, the combination of the guard and the transaction serve to guarantee that \texttt{emancipate}'s precondition is satisfied.}
...
\xxx[am]{Maybe I'm stating the obvious here, but you can't merge when a guard separates them, since the later edit becomes legal only once that guard has become true (\cref{sec:sequences}).
\Retire{} is the example: its \Remove{} cannot join its \Quiesce{} in one transaction, because the subtree has to drain in between.
Beyond that we make no claim.
Zhiyuan will need to back me up, but I believe that two edits with no guard between them (or with the \emph{same} guard) can share a transaction.}
\xxx[zhiyuan]{Compiler's duty is to insert guard seems correct to me. yes, i'd see instruction sequence as the following structure: transaction1(add, policy-update, add, ...); guard;transaction2(...).
To make the point general, I think generating the transactional sequence is compiler's duty.}
...
The word becomes relevant only at \texttt{undesignate}, which retargets the parent's index \texttt{k} without telling the parent, and reusing \texttt{k} preserves the parent's per-arm metadata for that slot (\cref{sec:lower-undesignate}).
...
A PE stores its \texttt{Assoc} set, the flows it admits; a \texttt{Map} naming the PIFO each admitted flow enters and the PE that holds it; and the designated-survivor link of \cref{sec:hw-survivor}.
...
The classification and survivor opcodes are thus the ones that write a PE's tables.
The lifecycle opcodes reach a PE too, since \texttt{spawn} and \texttt{gc} claim and release its PIFO storage.
The structure opcodes are the compiler's own bookkeeping, and \texttt{change\_arity} reaches no PE at all.

\xxx[as]{This is what remains of the old compiler section.}
\xxx[am]{That's okay, but I kinda feel that some text matching \zz's parts to the ISA should come back. It was the only place \cref{sec:controls}'s model met this ISA, my note in \cref{sec:controls} still points here for it, and \texttt{Assoc} is otherwise never defined.
Proposal: ``A node's \(\mathit{z}.\mathit{admit}\) is its \texttt{Assoc} set, its \(\mathit{z}.\mathit{serves}\) is its \texttt{Map} table, its \(\mathit{z}.\mathit{rank}\) is the ranker that \texttt{set\_policy} installs and \texttt{discipline(v)} reads back, and its \(\mathit{z}.\mathit{meta}\) is the metadata that \texttt{set\_arm\_meta} writes.
Each of \zz's parts is thus an opcode's target, and none of them is state that \texttt{push} or \texttt{pop} advances.''}
\xxx[as]{OK, sounds good. I would remove the last clause (``none of them is state that push or pop advances'') because I think it sounds like a bit of a non sequitur.}
\xxx[zhiyuan to as]{it's a good place to put "it's compiler's job to geneate valid sequence"?}
...
\xxx[zhiyuan]{Just need to update based on the discussion. if we want a separation between brain and mappers, we can refer to mappers as Lookup-Tables.}
...
\xxx[zhiyuan]{and update the previous active LUTs based by replay the commands for the commit in the background.}

A swap takes a single cycle, so it is straightforward to make it atomic within a PE,
...
\xxx[am]{Yes, they are non-transactional, and we do not claim the whole transaction applies atomically. What we claim is that the non-atomic part cannot be observed. The rank-computing writes are applied as the controller reaches them, while the classification writes are held for the commit. In \Add{} and \Designate{} those writes land only on a node or a slot that no packet can reach until the classification flip, and \cref{sec:lower-add} does put the two walks last. \Quiesce{}, \Remove{} and \Undesignate{} issue no rank-computing write at all. \ChangeMeta{} is the only one that writes a live arm, and it is safe for a different reason: it is a single write, so a \push{} reads either the old value or the new one, and ranks are frozen at \push{} time (\cref{sec:nondisruption}) so the backlog is unaffected either way. My note below asks Zhiyuan to confirm the \ChangeMeta{} case.}
Also, the rank computation must accept incremental updates rather than wholesale replacement of a live scheduling policy.
Our controller accepts a commit's rank-computing writes along with the rest of its instructions, and applies them as it reaches them rather than holding them until the commit installs.
That is the payoff for a narrow transactional scope: only the state that routes a packet has to install in a single instant.

\xxx[AM to Zhiyuan]{This paragraph is now the subsection's opening claim, per your ``we minimize the transactional scope''.
The \ChangeMeta{} sentence is the one case your two-way split does not cover, since it re-tunes an arm that is live and backlogged, and the argument I give is that \ChangeMeta{} lowers to a transaction holding that one write and nothing else.
...
\texttt{v}'s PIFOs, and those of everything beneath it, may be released only once the subtree is empty, and a second commit is guarded on the same emptiness predicate (\cref{sec:hw-commit}).
...
Say also which of your six control commands cost more than one cycle, and note that the counts include the Brain commands, which do not install atomically.}
...
\xxx[AM to Zhiyuan]{You told me the guarded garbage collection already exists in hardware and that you would add its evaluation line.
\cref{sec:hw-survivor} describes it as a second commit guarded on the same emptiness predicate, so the line belongs here.}
...
Rio reaches per-packet consistency by another means: a commit installs in a single cycle at each PE, so a packet meets one configuration or the other (\cref{sec:hw-commit}).
...
Rio runs above any of them and asks for one property: commits that are atomic with respect to traffic (\cref{sec:hw-commit}).
...
\paragraph{A gallery of the three equivalences}
...
\Add{} comes first, so that the six can be read together; \cref{sec:hw-isa} gives that same lowering in prose.
...
\texttt{spawn}'s elided \texttt{pe} argument is the PE at \texttt{v}'s depth (\cref{sec:hw-substrate}).
...
A switch with \(P\) output ports has \(P\) separate trees, and each is wrapped at the ISA in a reserved 1-arm PIFO \texttt{port\_root} whose sole child, reached through the index \texttt{port\_step}, is the actual tree root.
...
\texttt{change\_arity} creates that slot, which is the lowering's counterpart of \(\mathit{init\_slot\_state}_{D}\) in \cref{sec:add}.
