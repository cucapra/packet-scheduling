...
The control plane updates the scheduling policy by sending a \emph{transaction} consisting of a sequence of these instructions.
\xxx[as]{I have changed the noun ``commit'' to ``transaction,'' which I think will be less surprising.}
...
Therefore, each edit $\delta$ compiles to one transaction.
...
\xxx[as]{Is that true? I sorta just made this up... but we do need an example of the preconditions that we assume (if any exist).}
It is the compiler's responsibility to enforce these preconditions.
\xxx[as]{Just checking: are there any requirements on what can go in a single transaction? (Are there two things that you can do in separate transactions but not all together in one?) If so, this would be the place to bring it up.}

\xxx[as]{I admit I don't understand this subtlety. It sounds like a straightforward description of what it does. What is subtle about this? (Perhaps we can just delete this paragraph?)}
...
\xxx[as]{How can it be true that the PE choice is the hardware's, if \texttt{spawn(v, pe)} seems to direct the hardware which PE to choose?}
A typical choice is the depth-by-PE layout of \cref{sec:background}, under which siblings share a PE.
\xxx[as]{I think we should just assume this is always the case (in which we do not need the preceding sentence, and perhaps this whole paragraph).}
The planner consults \texttt{pe(path)} for a \texttt{spawn}'s \texttt{pe} argument and cannot otherwise observe it.



\subsection{Atomic Policy Updates}\label{sec:hw-commit}
