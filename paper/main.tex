...
Rio implements atomic updates by double-buffering the match--action tables (MATs) that make up the policy configuration.
These MATs control the association between flows and PIFOs and the parent--child relationships among PIFOs.
Configuration instructions affect the inactive MATs,
and committing a transaction swaps the active and inactive MATs.

It is straightforward to make these swaps atomic within a PE,
but transactions can affect multiple PEs.
We \emph{delay-align} the transaction commits across PEs according to the PIFO tree topology:
...
\xxx[as]{I still don't get what we are saying about disciplines/rank assignment functions and metadata. are those double buffered}
...
\xxx[as]{In other words: you are saying that instructions like \texttt{set\_policy} are applied immediately. I do not yet see a clear argument for why this is OK.}
...
\xxx[as]{seems like a nice comparison, but what is ``the forwarding pipeline''}
...
\xxx[as]{Does this mean that every transaction can include at most one \texttt{set\_arm\_meta} instruction? If so, let's say that in the ISA section.}



\xxx[as]{say this shorter: the interface accepts transactions, and can pause before applying them to wait for something to empty}
