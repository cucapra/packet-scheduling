...
In cycle-accurate simulation, Rio never pauses the scheduler to install a transition, and an unaffected flow suffers only a single cycle of delay.
Pausing the scheduler to install the same change costs that unaffected flow more than a thousand cycles.
On an FPGA, the two extensions add under 9\% to the scheduler's lookup tables and flip-flops (\cref{sec:eval}).
...
\Cref{sec:eval} returns to this example in cycle-accurate simulation, where the transition above keeps the link full and leaves \textsf{zoom} undisturbed from one end of \(p_\text{temp}\) to the other, and where both additions synthesize onto an FPGA.
