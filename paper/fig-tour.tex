...
% dashed is the route not taken, and shading is freshly built material.
\definecolor{actcol}{HTML}{D55E00}
% Gray parenthetical pointers into the paper. \ref keeps the numbers honest.
\newcommand{\secptr}[1]{\textup{\textcolor{black!55}{(\S\ref{#1})}}}
\begin{figure*}[t]
\centering
  \begin{tikzpicture}[
  ...
  \begin{tikzpicture}[x=1cm,y=1cm]
    ...
    \node[pn] (b3)  at ( 0.40,-1.60) {\textsf{gmail}\textsubscript{old}};
    \node[pn] (rr)  at ( 2.20,-1.60) {\(\RR\)};
    \node[pn] (rrA) at ( 1.60,-2.40) {\textsf{gmail}\textsubscript{new}};
  ...
  % ===== headings: named policies are the ones an operator wrote =====
  \node[hd] at (P1.north) {\(p_1\)\,\secptr{sec:model}};
  \node[hd] at (P2.north) {\(p_\text{temp}\)};
  \node[hd] at (P3.north) {\(p_2\)};

  % ============================ controls ============================
  \node[ctlbox] (C1)  at (P1.south |- 0,-4.15) {\(C_1\)};
  \node[sem, anchor=east] at ($(C1.west)-(0.13,0)$) {\secptr{sec:semantics}};
  \node[ctlbox] (C2m) at (P2.south |- 0,-4.15) {\emph{transitional control}};
  \node[ctlbox] (C2)  at (P3.south |- 0,-4.15) {\(C_2\)};
  ...
  \draw[says,->] ($(P1.north east)+(0.10,-0.42)$) -- ($(P2.north west)+(-0.10,-0.42)$)
    ...
    node[gd, midway, below=1pt] {\secptr{sec:model}};
  ...
  \draw[does,->] ($(C1.east)+(0.10,0)$) -- ($(C2m.west)+(-0.10,0)$)
    ...
    node[gd, midway, below=1pt] {immediately \secptr{sec:semantics}};
  \draw[does,->] ($(C2m.east)+(0.10,0)$) -- ($(C2.west)+(-0.10,0)$)
    node[act, midway, above=1pt] {\(\csem{\Undesignate}\)}
    node[gd, midway, below=1pt] {once \textsf{gmail}\textsubscript{old} drains \secptr{sec:correctness}};
  \draw[does,->] (P1.south) -- (C1.north)
    node[act, midway, anchor=east, xshift=-2pt, align=right]
      {\(\lceil \cdot \rceil\)\\\secptr{sec:semantics}};
  \draw[does,->] (C2m.north) -- (P2.south)
    node[act, midway, anchor=west, xshift=2pt] {\(\lfloor \cdot \rfloor\)}
    node[gd, midway, anchor=east, xshift=-3pt] {what the operator\\is told \secptr{sec:correctness}};

  % ================ dashed: the route not taken ================
  \draw[notaken,->] (P3.south) -- (C2.north)
    node[gd, midway, anchor=east, xshift=-4pt] {what a recompile\\would have built \secptr{sec:correctness}};

  \end{tikzpicture}
  \caption{An example transition between two packet scheduling policies, $p_1$ and $p_2$. Rio uses an intermediate policy, $p_\text{temp}$, to implement the transition in two atomic steps.
  The top row is the policy in force and the bottom row is the control that realizes it.
  $\lceil \cdot \rceil$ compiles a policy to a control and $\lfloor \cdot \rfloor$ reads one back.
  Shading shows material Rio builds fresh.}
  \label{fig:tour}
\end{figure*}
