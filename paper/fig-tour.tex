...
\centering
  ...
  \draw[does,->] ($(C2m.east)+(0.10,0)$) -- ($(C2.west)+(-0.10,0)$)
    ...
    node[gd, midway, below=1pt] {once \textsf{gmail}\textsubscript{old} drains};
  ...
  \draw[does,->] (C2m.north) -- (P2.south)
    ...
    node[gd, midway, anchor=east, xshift=-3pt] {what the operator\\is told};
  ...
  The top row is the policy in force and the bottom row is the control the hardware holds.
  Rio compiles $p_1$ to an initial PIFO tree control, $C_1 := \lceil p_1 \rceil$, and then compiles each edit into an atomic update of the hardware state.
  The operation $\lfloor \cdot \rfloor$ reads a policy back off a running control.
