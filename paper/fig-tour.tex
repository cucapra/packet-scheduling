...
\centering
  ...
  \end{tikzpicture}
  \caption{An example transition between two packet scheduling policies, $p_1$ and $p_2$. Rio uses an intermediate policy, $p_\text{temp}$, to implement the transition in two steps. Each transition, from $p_1$ to $p_\text{temp}$ and from $p_\text{temp}$ to $p_2$, is atomic. Rio compiles $p_1$ to an initial PIFO tree control, $C_1$, and each edit into an update of the hardware state.}
  \label{fig:tour}
\end{figure*}
