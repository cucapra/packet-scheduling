...
%% ===========================================================================
%% 2026-09-06, later: the planner section is cut.
%% ===========================================================================
%%
%% AM: "Now let's do the section about the planner (the new \S7). I agree with
%% the xxx that it can be cut."
%%
%% Adrian's \xxx, preserved at the top of R8 below, named the three points that
%% had to survive: the planner exists but is "just" a heuristic with no
%% optimality claim; it produces simple transitions when the difference is
%% confined (AM's correction: confined, not small); and even when a Strict*
%% transition is needed it stays local when it can. Those three are now
%% \S7.1, ten lines at the head of the hardware section, which is the
%% destination Adrian asked for.
%%
%% What the cut costs, checked before it was made:
%%
%%   Nothing in the completeness argument. \S6.4 already carries the witness:
%%   "Replace([], p_2) discharges the completeness criterion of \S4.2." That is
%%   why sec:fallback no longer had to be rescued out of the section.
%%
%%   Nothing in the appendix. app:planner-emits is self-contained: it defines
%%   normalization itself and cites sec:equivalences for the canonical
%%   representative, so \S6.1's canonical-representative sentence keeps a
%%   consumer and Adrian's \xxx on it is answered.
%%
%%   The pol-level check is gone with sec:imperative, its only real consumer.
%%   AM had flagged it ("not sure we're going to keep any pol-level check") and
%%   the cut settles it. R9 is the one sentence that had been left in \S6.4 to
%%   keep the section's four references landing.
%%
%%   fig:planner-add-vs-replace is gone. It was the clearest picture of
%%   confinement in the paper, an Add-in-place drawn beside a root-level
%%   Replace, and it is inside R8 if it is wanted back. A stale \cref to it in
%%   a fig-tour.tex comment was reworded.
%%
%% Nine crefs were repointed: five sec:fallback and two more in the appendix to
%% sec:sequences, one sec:fallback to sec:planner, and app:planner-emits's own
%% "this appendix expands" to sec:planner. \S1's lowering cref moved from
%% sec:hardware to sec:hw-compile, so its two pointers now name the two halves
%% of one section rather than a section and itself.
%%
%% To restore: R8 is a whole unit and goes back between \S6 and the hardware
%% section, at which point \S7.1 should be deleted and R9 put back into \S6.4.
%%
%% ===========================================================================


% ---------------------------------------------------------------------------
% R8. \S7 in full, ``The Transition Planner''
% From: main.tex, between \S6 (Correctness) and the hardware section.
% Cut 2026-09-06. Contains sec:fallback (the always-available Replace, the
% footprint definition, confinement, and fig:planner-add-vs-replace),
% sec:planner-emits (the case analysis in brief), and sec:imperative
% (hand-authoring and the pol-level check that admitted it).
% ---------------------------------------------------------------------------
...
% ---------------------------------------------------------------------------
% R9. \S6.4's pol-level check sentence
% From: main.tex sec:sequences, closing the ``Both theorems hold'' paragraph.
% Cut 2026-09-06 with R8. It was already the compressed remnant of a
% four-sentence paragraph, kept alive only so that the planner section's four
% references to the check still landed. With sec:imperative gone the check has
% no consumer.
% ---------------------------------------------------------------------------
...
% ---------------------------------------------------------------------------
% R10. The argument for keeping the non-disruption theorem
% From: main.tex sec:nondisruption, an \xxx[AM to AS] note.
% Cut 2026-09-06. It is spent: Adrian read it and conceded in his own note at
% the head of the subsection, "your argument below about why to keep this
% theorem is good. Let's keep it." The theorem stays; only the argument for it
% leaves, since the paper does not need to argue with a settled reviewer.
% The Reset(p_2) construction is kept here because it is the sharpest form of
% the case, and it would have to be rebuilt from scratch if the question of
% dropping the theorem is ever reopened. Region 8 above holds an earlier,
% pre-ChangeRoot-cut sentence of the same note; this is the version Adrian
% actually read and conceded to.
% ---------------------------------------------------------------------------
...
% ---------------------------------------------------------------------------
% R11. Appendix F, ``Inside the Transitional Interval''
% From: main.tex, the last appendix, label app:ptemp, with fig:eval-ptemp
% (fig-eval-ptemp.pdf, still on disk and still in the Makefile's DEPS).
% Cut 2026-09-09 on AM's instruction, during the appendix consistency pass.
% It restated \S2's p_1/p_temp/p_2 and \S8's traffic mix, then read one figure:
% spotify reaches first service 709 cycles after the request against the
% stop-the-world baseline's 1408. That number lives nowhere else in the paper.
% Cutting it also closed review item 18, which was that the figure shows
% spotify flat at zero for the width of the band while \S8 says the interval
% ``costs the traffic nothing''.
% \S8's pointer to it went too, one sentence in ``The value of a survivor
% link'': ``It is also why spotify waits for the drain in \cref{app:ptemp}.''
% ---------------------------------------------------------------------------
