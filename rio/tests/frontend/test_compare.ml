...
let make_compare_test name file1 file2 (expected_diff : t) =
...
(* Helper for the "give up" cases below: [Compare] couldn't break the
   diff down at depth [List.length path], so it emits a singleton list
...
let same =
  [
    make_compare_test "same program twice" "strict_ABC" "strict_ABC" [];
    make_compare_test "merely jumbled in RR" "rr_ABC" "rr_BAC" [];
    make_compare_test "merely jumbled in WFQ" "wfq_ABC" "wfq_ABC_jumbled" [];
    ...
    make_compare_test "complex tree with an rr-reordering deep down"
      "complex_tree" "complex_tree_swap_rr_arms" [];
  ]
...
(* OneArmAdded fires on a single-arm insertion at any position of a
   ...
   children). A WFQ-arm-add lands as a *two-element* list
   [[OneArmAdded; WeightChanged]] because the new slot's weight can't ride
   along on [arm_diff]; see [wfq_arm_added] below. Multi-arm insertions
   instead "give up" to a wholesale-replace [OneArmReplaced] (see
   [verydiff_combos]). The [path] inside [arm_diff] is the new arm's full
   position from the root of [next]. *)
let one_arm_added =
  [
...
(* WFQ-arm-add: Compare emits the pair [OneArmAdded; WeightChanged] at the
   same path. The IR composes the pair by splicing the new slot in (with
   a placeholder weight) and then immediately overwriting that weight. *)
let wfq_arm_added =
  [
    (* wfq_BA = wfq[(B,1),(A,2)] → normalize → wfq[(A,2),(B,1)].
       wfq_ABC = wfq[(A,2),(B,1),(C,3)]. Insertion of (C, 3) at index 2. *)
    make_compare_test "WFQ with arm added" "wfq_BA" "wfq_ABC"
      [
        OneArmAdded { path = [ 2 ]; arm = Policy.FIFO "C" };
        WeightChanged { path = [ 2 ]; new_weight = 3.0 };
      ];
    (* complex_tree_partial = wfq[(strict[A,B,C], 1), (union[G,H], 3)] →
       normalize sorts WFQ pairs by (policy, weight): UNION < SP, so
       prev becomes wfq[(union[G,H],3), (strict[A,B,C],1)].
       complex_tree adds the rr[F,E,D] arm with weight 2 → after normalize
       it sits at index 2 with arm = RR[D,E,F] (RR children sorted). *)
    make_compare_test "complex tree fill in missing arm" "complex_tree_partial"
      ...
      [
        ...
        WeightChanged { path = [ 2 ]; new_weight = 2.0 };
      ];
  ]
...
(* A menu of cases where the diff is a *combination* of changes each of
   ...
   of the divergence and emits a singleton
   [[OneArmReplaced { path; arm = next_at_path }]] — the IR will replace
   that subtree wholesale via [Designate]. As [Compare] gets smarter
   (e.g., learns to emit longer lists of edits), entries here will
   migrate to more precise variants — that's already happened for
   WFQ-arm-add (now in [wfq_arm_added]). Each remaining entry's comment
   names the chain of legal changes that together overwhelm the
   analyzer. *)
let verydiff_combos =
  [
