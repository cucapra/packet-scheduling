...
class StandalonePlotTest(unittest.TestCase):
    def test_saved_result_scripts_run_in_isolated_folders(self):
        # Workflow snapshots also contain repository helpers named *_plot.py.
        # Exported standalone artifacts use plot.py or a legacy *-plot.py name.
        scripts = sorted(path for path in (ROOT / "experiment-results").rglob("*plot.py")
                         if path.name == "plot.py" or path.name.endswith("-plot.py"))
    ...
    def test_backfill_leaves_resource_synthesis_artifacts_unchanged(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            run = root / "shared-fifo-overhead"
            figure = run / "r1-fixed/figures/resource-table"
            figure.mkdir(parents=True)
            (run / "experiment-config.json").write_text('{"plot":{"dpi":180}}\n')
            (run / "r1-fixed/experiment-config.json").write_text('{"plot":{"dpi":180}}\n')
            (figure / "data.csv").write_text("platform,resource,static,replay\nvivado,bram36_tiles,1,2\n")
            (figure / "figure.svg").write_text("<svg>Resource comparison</svg>\n")
            before = {p.relative_to(root):p.read_bytes() for p in root.rglob("*") if p.is_file()}
            self.assertEqual(export_saved_figures(root), [])
