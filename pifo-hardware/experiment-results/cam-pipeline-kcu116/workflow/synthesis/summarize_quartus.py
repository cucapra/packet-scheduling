...
def zero_memory_usage(tables):
    """Recognize omitted zero-memory rows only with complete hierarchy evidence."""
    usage = next((rows for name, rows in tables.items()
                  if name.startswith("Synthesis Resource Usage Summary")), [])
    values = {row[0]: row[1] for row in usage}
    if "Total block memory bits" in values or "Total MLAB memory bits" in values:
        return None
    # A missing summary alone is not evidence of zero: oversized designs can
    # omit it too. Require an explicit zero at every entity, including the root,
    # and no inferred RAM instances.
    if any(rows[1:] for name, rows in tables.items() if "RAM Summary" in name):
        return None
    hierarchy = next((rows for name, rows in tables.items()
                      if "Resource Utilization by Entity" in name), [])
    if not hierarchy:
        return None
    entities = [dict(zip(hierarchy[0], row)) for row in hierarchy[1:]]
    if (not any(row.get("Full Hierarchy Name") == "|" for row in entities)
            or any(row.get("Block Memory Bits") != "0" for row in entities)):
        return None
    return {"block_memory_bits": 0, "mlab_memory_bits": 0,
            "hierarchy_entities_checked": len(entities), "ram_instances": 0,
            "note": "Quartus omits the memory summary for this RAM-free design. Every hierarchy entity explicitly reports zero Block Memory Bits and there are no inferred RAM instances."}
...
def summarize(report):
    ...
    if recovered:
        ...
        zero = zero_memory_usage(selected)
        if zero:
            result["zero_memory_usage"] = zero
            result["resource_notes"] = [zero["note"]]
    return result
