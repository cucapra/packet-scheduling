#!/usr/bin/env python3
"""VCU118 staged-CAM sweep: eight RIO cases and four matching native PIFOs."""
import argparse
...
OUT = PROJECT / 'experiment-results/cam-pipeline-vcu118'
BUILD_ROOT = Path('/data/work/rio-synthesis/cam-pipeline-vcu118-20260909')
VERSION = 'cam-pipeline-vcu118-v1'
...
def name(flows, mode):
    return (f'pifo-v{flows}-c1024-vcu118' if mode == 'pifo'
            else f'pe5-v{flows}-c1024-q256-p128-{mode}-vcu118')


def command(root, flows, mode):
    cmd = [sys.executable, str(PROJECT / 'synthesis/run.py'), '--tool', 'vivado',
           '--board', 'vcu118', '--build-root', str(root), '--name', name(flows, mode),
...
def verify_inputs(build):
    ...
    assert m['board_part_matches_target'] and m['board_reference']['board_name'] == 'vcu118'
...
def archive_vivado(build, output):
    ...
    if result['status'] == 'synthesis_complete':
        ...
        for resource, row in [('distributed_ram_luts', 'LUT as Distributed RAM'),
            ...
            result['resources'][resource] = native[row]['Used']
...
def collect(root):
    board = vivado_board_info(vivado_root(None), 'vcu118')
    assert board, 'VCU118 board definition is required'
    results, statuses = {}, []
    for flows in FLOWS:
    ...
    target = dict(TARGETS['vivado'], part=board['part'], label='VCU118 XCVU9P', available=None)
    if results:
...
def archive_inputs_and_workflow(root):
    ...
    board = vivado_board_info(vivado_root(None), 'vcu118')
    shutil.copy2(board['definition'], OUT / 'vcu118-board.xml')
            ...
            for flows in reversed(FLOWS):
                for mode in ('replay', 'static', 'pifo'):
                    ...
                    if manifest(build).get('rtl_complete'):
                        ...
                        execute(command(root, flows, mode) + ['--generate-only'],
                                OUT / 'logs' / (build.name + '-generate.log'))
                        verify_inputs(build)
            collect(root)
