...
module shared_control_fifo_tb;
  ...
  reg [44:0] expected [0:32767], epoch [0:1023], held;
  reg expected_replay [0:32767];
  reg stalled=0;
  ...
  initial begin
    ...
    for(integer k=0;k<64;k=k+1) send(k%4==0 ? 2 : 4+(k%3),k);
    drain();
    ...
    for(integer seed=1;seed<=3;seed=seed+1) begin
      ...
      for(integer batch=0;batch<180;batch=batch+1) begin
        for(integer k=0;k<batch%D;k=k+1) begin
          ...
          else send(batch%4==0 ? 2 : 4+(k%3),batch*17+k);
        end
        send(3);
        if(batch%7==0) send(3);
      end
