...
/**
  ...
  */
case class TransactionalMapper(inputWidth: Int, outputWidth: Int)
...
case class PifoEngine(config: EngineConfig) extends Component {
  ...
  val pifos: PifoCore = config.pifoBackend match {
    ...
    case "external" => new ExternalPifoCore(config)
  }
  if (config.pifoBackend == "external") {
    io.pifo <> pifos.asInstanceOf[ExternalPifoCore].boundary
  }
  ...
  val enque = new Area {
    ...
    val enqueMapper: ConfigurableMapper = if (config.dynamicConfig && config.mapperSync == "replay")
      ReplayMapper(config.vpifoIdWidth, config.vpifoIdWidth)
    else if (config.dynamicConfig)
      TransactionalMapper(config.vpifoIdWidth, config.vpifoIdWidth)
  ...
  val deque = new Area {
    ...
    val dequeMapper: ConfigurableMapper = if (config.dynamicConfig && config.mapperSync == "replay") ReplayMapper(
      config.vpifoIdWidth + config.flowIdWidth, config.flowIdWidth
    ) else if (config.dynamicConfig) TransactionalMapper(
  ...
  // In the ordinary-table baseline a commit is a consumed no-op. In particular,
  // it cannot arm drain rewrites or alter lookup visibility.
