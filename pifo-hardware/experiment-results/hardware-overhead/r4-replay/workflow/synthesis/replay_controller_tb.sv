...
`define CHECK_PE(N) \
  ...
  initial begin
    ...
    $display("REPLAY_CONTROLLER_PASS cycles=%0d commits=%0d mapper_updates=%0d busy_cycles=%0d lookups=%0d lookups_during_replay=%0d lookups_on_swap=%0d queued_epoch_updates=%0d",cycles,swaps,accepted_updates,busy_cycles,checked_reads,reads_during_replay,reads_on_swap,queued_epoch_updates);
    $finish;
  end
endmodule
