...
def execute_case(config, flows, platform, sources, root, output):
    ...
    shutil.copy2(build/'rtl/PifoMesh.v', target/'pifo-component.v')
...
def main():
    ...
    (output/'execution.json').write_text(json.dumps(dict(argv=sys.argv, source_root=str(args.source_root),
        build_root=str(args.build_root), jobs_per_vendor=config['jobs_per_vendor'], implementation=False), indent=2)+'\n')
    results=[]
    def publish():
    ...
    with ThreadPoolExecutor(max_workers=config['jobs_per_vendor']['quartus']) as qp, \
        ...
        futures=[pools[p].submit(execute_case,config,f,p,args.source_root,args.build_root,output)
                 for p in config['platforms'] for f in config['vflows']]
