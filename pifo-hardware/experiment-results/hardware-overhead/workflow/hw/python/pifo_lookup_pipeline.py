...
from pifo_hardware_overhead import PROJECT, RESOURCE_NAMES, archive, resources, write_csv, count_text
...
def render(output, config):
    ...
    for platform in config["platforms"]:
        ...
        for resource, baseline in pair[short].items():
            ...
            comparisons.append({"platform": platform, "resource": resource,
                                ...
                                "percent_change": 100 * (pipelined - baseline) / baseline if baseline else ""})
    fields = ["platform", "resource", "short_latency", "long_latency", "short_resource", "long_resource", "absolute_change", "percent_change"]
    write_csv(output / "comparison.csv", comparisons, fields)
    lines = [f'# {config["title"]}', "", config["scope"], "",
             f'Both accept one request per cycle; output latency is {short} or {long} cycles.',
    ...
    for row in comparisons:
        ...
        values = [row["platform"].capitalize(), RESOURCE_NAMES[row["resource"]][0],
                  ...
                  count_text(row["absolute_change"], True), percent]
        lines.append('| ' + ' | '.join(values) + ' |')
        if row["resource"] not in ("mlab_memory_bits", "dsp_blocks", "uram288_blocks", "lutram_luts"):
            cells.append(values)
    lines += ["", 'The validation archive checks data/valid alignment, bubbles, bursts, collisions, highest addresses, and reset during outstanding reads.', ""]
    ...
    ax.axis("off")
    if cells:
        table = ax.table(cellText=cells, colLabels=["Platform", "Resource", f"{short} cycle(s)", f"{long} cycles", "Difference", "Change"],
