...
object GeneratePifoMesh {
  def main(args: Array[String]): Unit = {
    require(args.length >= 5 && args.length <= 7,
      "Usage: GeneratePifoMesh OUTPUT_DIR NUM_ENGINES NUM_VPIFOS FIFO_DEPTH PRIORITY_BITS [house|stock|external] [dynamic|static]")
    ...
    require(Set("dynamic", "static").contains(configuration))
    ...
    val config = EngineConfig(
      ...
      pifoBackend = backend,
      dynamicConfig = configuration == "dynamic"
    )
