// Isolated ordinary lookup; throughput one request/cycle, latency 1 cycles.
module PifoMesh (
  ...
  always @(posedge clk) begin
    ...
    for (integer k=1; k<1; k=k+1) data_pipe[k] <= data_pipe[k-1];
    if (reset) valid_pipe <= 0;
    else begin
      valid_pipe[0] <= read_valid;
      for (integer k=1; k<1; k=k+1) valid_pipe[k] <= valid_pipe[k-1];
    end
  end
  assign response_valid = valid_pipe[0];
  assign response_data = data_pipe[0];
endmodule
