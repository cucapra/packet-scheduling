...
.venv/bin/python hw/python/pifo_simulator.py \
  ...
  --queue-depth 4096 --link-bytes-per-cycle 16 --max-cycles 50000 --evaluation-hardware --verilator
```
...
not the front-rewrite whole-tree implementation now also available on this branch. This distinction
...
guards every relevant FIFO, not only a parent whose last packet may still be descending.
...
panels mark every commit's start, acceptance, own replay-ready time and shared old-tree drain, with distinct
blue/amber/green backgrounds for install, cleanup and reclamation.
