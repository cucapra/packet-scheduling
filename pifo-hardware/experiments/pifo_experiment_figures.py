...
from pifo_config import generate_distributed_requests
...
from pifo_tree_compiler import TreeMoveProgram, write_tree_move_program
from pifo_transaction_program import HardwareShape
...
def run_experiment(args: argparse.Namespace) -> None:
    ...
    hardware = HardwareShape(
        simulation.num_engines, simulation.num_vpifos, simulation.max_packet_priority,
        simulation.fifo_depth, simulation.prefetch_buffer_depth,
    )
    write_tree_move_program(
        tree_move, TreeMoveProgram(hardware, config.initial_tree, config.reconfiguration)
    )
    write_traffic_program(
        traffic,
        TrafficProgram(
    ...
    _paths.run_script(
        _paths.CORE_PYTHON / "pifo_tree_compiler.py",
        "--input",
        tree_move,
        "--output",
        transactions,
    ...
    _paths.run_script(
        _paths.CORE_PYTHON / "pifo_simulator.py",
        "--transactions",
        transactions,
        "--traffic",
        traffic,
    ...
    _paths.run_script(
        Path(__file__).with_name("pifo_bandwidth_figure.py"),
        *common_figure_args,
        "--output-dir",
        bandwidth_dir,
        "--link-bytes-per-cycle",
    ...
    _paths.run_script(
        Path(__file__).with_name("pifo_packet_scatter_figure.py"),
        *common_figure_args,
        "--output-dir",
        scatter_dir,
    )
