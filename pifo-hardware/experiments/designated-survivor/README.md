...
The fourth PE also increases engine-ID width in this implementation: post-mapper banks now have 512 entries rather than 256. Their synchronization time is included in configuration finish/reclamation costs, not treated as another packet outage.
...
After WaitPifoEmpty, a second package stops creating wrapper entries and publishes the survivor as root. Following bank synchronization, ClearPifoEngine invalidates the detached wrapper's remaining tokens by clearing its counters. This is one-cycle logical reclamation, not a sequential token deletion. The packet FIFOs on PE 3 remain intact and surviving packets must still complete. Validation for this modification uses compiler/unit checks and the experiment runs themselves, not standalone RTL regression tests.
