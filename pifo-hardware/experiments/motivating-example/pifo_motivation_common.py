...
HARDWARE_ROOT = _paths.HARDWARE_ROOT
...
def run_case(
    ...
    if not render_only:
        ...
        _paths.run_script(_paths.CORE_PYTHON / "pifo_simulator.py", *simulator_args)
    ...
    _paths.run_script(
        Path(__file__).with_name("pifo_motivation_throughput_figure.py"),
        *common_figure_args,
        "--output-dir",
    ...
    _paths.run_script(
        Path(__file__).with_name("pifo_motivation_scatter_figure.py"),
        *common_figure_args,
        "--output-dir",
...
def run_comparisons(output_root: Path) -> None:
    ...
    _paths.run_script(
        Path(__file__).with_name("pifo_motivation_compare_scatter.py"),
        "--r2-dir",
    ...
    _paths.run_script(
        Path(__file__).with_name("pifo_motivation_compare_throughput.py"),
        "--r3-dir",
        output_root / "r3-whole-tree",
        "--r4-dir",
        output_root / "r4-confined",
