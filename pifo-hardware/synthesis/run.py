...
def main():
    ...
    parser.add_argument("--pifo-backend", choices=("house", "stock"), default="house",
                        help="PIFO core implementation; stock is experimental (see STOCK_PIFO_RESULTS.md)")
    ...
    metadata = {
        ...
        "scope": "PifoMesh, all PEs, crossbar, configuration/commit controller",
        ...
        "logical_storage_bits": {
            "sorted_entry_registers": args.engines * args.entries_per_pe * (v + args.priority_bits + token_bits),
            "flow_state_tables": args.engines * pair_depth * 32,
            "post_mapper_both_banks": args.engines * pair_depth * token_bits * 2,
            "pre_mapper_both_banks": args.engines * args.vpifos * v * 2,
            ...
            "front_rewrite_registers": args.engines * args.vpifos * (v + 4),
    ...
    try:
        ...
        if reuse_build is not None and reuse_build != build:
            ...
            command = (f'runMain rio.GeneratePifoMesh "{rtl}" {args.engines} '
                       f'{args.vpifos} {hardware["fifo_depth_parameter"]} {args.priority_bits} {args.pifo_backend}')
        ...
        if args.tool == "quartus":
            run([root / "bin/quartus_sh", "-t", HERE / "create_project.tcl", build, part,
                 str(args.threads), str(1000 / args.clock_mhz)], build, build / "project.log", env)
