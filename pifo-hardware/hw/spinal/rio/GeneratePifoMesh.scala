...
object GeneratePifoMesh {
  def main(args: Array[String]): Unit = {
    require(args.length == 5 || args.length == 6,
      "Usage: GeneratePifoMesh OUTPUT_DIR NUM_ENGINES NUM_VPIFOS FIFO_DEPTH PRIORITY_BITS [house|stock]")
    ...
    println(s"PifoMesh: $engines PEs, $vpifos vPIFO IDs/PE, " +
      s"${vpifos * depth} shared entries/PE, $vpifos global flow IDs, $priorityBits rank bits, $backend PIFO")
  }
}
