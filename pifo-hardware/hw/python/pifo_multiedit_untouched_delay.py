#!/usr/bin/env python3
"""Figure B: untouched-flow delays, shared axes and p1 control overlays."""
...
def main():
    ...
    paths = {run: args.results / run for run in ("control", *runs)}
    ...
    rows = [{**p, "source": p["run"], "run": run} for run in runs
            ...
            if p["run"] in ("control", run) and p["flow"] in selected and p["pop_cycle"]]
    export(figure_dir(args, "untouched-delay"), "untouched", rows, packets, commit_rows(paths),
           {"runs": runs, "flows": [cfg["flow_labels"][str(f)] for f in cfg["untouched_flows"]],
            "titles": TITLES, "xlim": (0, cfg.get("plot_end_cycle", cfg["end_cycle"]))})
