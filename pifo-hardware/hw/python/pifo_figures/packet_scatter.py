...
def render_matplotlib(
    ...
    marker_values = [cycle for cycle, *_ in timeline_markers(event)]
    ...
    draw_timeline(axis, event, horizontal=True)
...
def render_svg(
    ...
    entries.append(("y = x", "#6baed6", None))
    entries.extend((label + " (background)", color, None) for _, _, color, label in timeline_spans(event))
    entries.extend((label, color, {"-": None, "--": "8,6", "-.": "8,4,2,4", ":": "3,5"}[style])
                   for _, color, style, label in timeline_markers(event))
    legend(
        svg,
        area.x + 18 * scale,
        area.y + 18 * scale,
