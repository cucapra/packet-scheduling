...
def export_saved_figures(results_root: Path) -> list[Path]:
    ...
    for data in sorted(results_root.glob("**/figures/*/data.csv")):
        ...
        # Resource-synthesis figures share this layout but have no packet events.
        if not (run_root / "reconfiguration-events.csv").is_file():
            continue
    ...
    for config in sorted(results_root.glob("*/experiment-config.json")):
        root = config.parent
        if not any((root / name).is_file() for name in ("bandwidth.csv", "packet-times.csv")):
            continue
