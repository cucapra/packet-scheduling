...
def compile_request(spec: dict, mechanism: str) -> tuple[TransactionProgram, dict]:
    ...
    def retirement(tree, placement):
        ...
        commands = []
        for tenant in tree:
            commands.append(cmd("WaitPifoEmpty", t, tenant["slot"]))
            for flow in tenant["flows"]:
                fid = flow["id"]
    ...
    transactions = []
    report = {"mechanism": mechanism, "unguarded_edits": [], "guarded_edits": [],
    ...
    if mechanism == "control":
        ...
        if cleanup:
            transactions.append(TimedTransaction(t1, "retire", tuple(cleanup + [commit])))
        ...
        cleanup = [cmd("WaitPifoEmpty", old_place[0], 1)]
        cleanup += [cmd("UpdateMapperPre", wrapper, fid) for fid in sorted(allflows(new))]
        cleanup += [cmd("UpdateRoot", new_place[0], 1), commit]
        transactions.append(TimedTransaction(t1, "collapse", tuple(cleanup)))
        transactions.append(TimedTransaction(t1, "reclaim", (
            cmd("ClearPifoEngine", wrapper), commit)))
    else:
